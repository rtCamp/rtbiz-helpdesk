...
if ( ! class_exists( 'Rtbiz_HD_i18n' ) ) {
	class Rtbiz_HD_i18n {
		...
		public function load_plugin_textdomain() {
			...
			$mofile_global = WP_LANG_DIR . '/' . RTBIZ_HD_TEXT_DOMAIN . '/' . $mofile;

			if ( file_exists( $mofile_global ) ) {
				// Look in global /wp-content/languages/rtbiz folder
				load_textdomain( RTBIZ_HD_TEXT_DOMAIN, $mofile_global );
			} elseif ( file_exists( $mofile_local ) ) {
				// Look in local /wp-content/plugins/rtbiz/languages/ folder
				load_textdomain( RTBIZ_HD_TEXT_DOMAIN, $mofile_local );
			} else {
