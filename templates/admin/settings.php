...
<ul class="subsubsub">
	<li>
		<a href='<?php echo esc_url( admin_url( 'edit.php?post_type=' .  Rt_HD_Module::$post_type . '&page=rthd-settings&type=googleApi&tab=admin-settings' ) ); ?>' <?php if ( $_REQUEST['type'] == 'googleApi' ) {
			echo " class='current'";
		} ?> ><?php _e( 'Google API' ); ?></a> |
	</li>
	<li>
		<a href='<?php echo esc_url( admin_url( 'edit.php?post_type=' . Rt_HD_Module::$post_type . '&page=rthd-settings&type=imapServers&tab=admin-settings' ) ); ?>' <?php if ( $_REQUEST['type'] == 'imapServers' ) {
			echo " class='current'";
		} ?> ><?php _e( 'IMAP Servers' ); ?></a> |
	</li>
	<li>
		<a href='<?php echo esc_url( admin_url( 'edit.php?post_type=' . Rt_HD_Module::$post_type . '&page=rthd-settings&type=systememails&tab=admin-settings' ) ); ?>' <?php if ( $_REQUEST['type'] == 'systememails' ) {
			echo " class='current'";
		} ?> ><?php _e( 'System Emails' ); ?></a> |
	</li>
	<li>
		<a href='<?php echo esc_url( admin_url( 'edit.php?post_type=' . Rt_HD_Module::$post_type . '&page=rthd-settings&type=other&tab=admin-settings' ) ); ?>' <?php if ( $_REQUEST['type'] == 'other' ) {
			echo " class='current'";
		} ?> ><?php _e( 'Other' ); ?></a></li>
</ul>

<form method="post"
      action='<?php echo esc_url( admin_url( 'edit.php?post_type=' . Rt_HD_Module::$post_type . '&page=rthd-settings&tab=admin-settings&type=' . $_REQUEST['type'] ) ); ?>'>
...
<?php if ( $_REQUEST['type'] == 'googleApi' ) {
	...
	<tr valign="top">
		...
		<td><input type="text" name="rthd_googleapi_clientid" id="rthd_googleapi_clientid"
		           value="<?php echo esc_attr( get_site_option( 'rthd_googleapi_clientid' ) ); ?>"/>
			...
			<p class="description">Create an app on <a target="_blank" href="https://code.google.com/apis/console">google
				...
				<b><?php echo esc_html( get_site_option( 'rthd_googleapi_redirecturl' ) ); ?></b>
		</td>
	</tr>
	<tr valign="top">
		...
		<td><input type="text" name="rthd_googleapi_clientsecret" id="rthd_googleapi_clientsecret"
		           value="<?php echo esc_attr( get_site_option( 'rthd_googleapi_clientsecret' ) ); ?>"/></td>
	</tr>
						...
						<tr valign="top">
							...
							<td>
								<select name="rthd_imap_servers[<?php echo esc_attr( $server->id ); ?>][incoming_imap_enc]">
									...
									<option
										value="ssl" <?php echo esc_attr( ( $server->incoming_imap_enc == 'ssl' ) ? 'selected="selected"' : '' ); ?>><?php _e( 'SSL' ); ?></option>
									<option
										value="tls" <?php echo esc_attr( ( $server->incoming_imap_enc == 'tls' ) ? 'selected="selected"' : '' ); ?>><?php _e( 'TLS' ); ?></option>
								</select>
							</td>
						</tr>
						<tr valign="top">
						...
						<tr valign="top">
							...
							<td>
								<select name="rthd_imap_servers[<?php echo esc_attr( $server->id ); ?>][outgoing_smtp_enc]">
									<option value=""><?php _e( 'Select Encryption Method' ); ?></option>
									<option
										value="ssl" <?php  echo esc_attr( ( $server->outgoing_smtp_enc == 'ssl' ) ? 'selected="selected"' : '' ); ?>><?php _e( 'SSL' ); ?></option>
									<option
										value="tls" <?php echo esc_attr( ( $server->outgoing_smtp_enc == 'tls' ) ? 'selected="selected"' : '' ); ?>><?php _e( 'TLS' ); ?></option>
								</select>
							</td>
						</tr>
					</table>
	...
	<?php
		...
		<tr valign="top">
			<th scope="row"><label
					for="<?php echo esc_attr( $slug ); ?>_system_email"><?php echo esc_attr( $rt_hd_module->name ). esc_attr( __( ' System Email' ) ); ?></label>
			</th>
			<td>
				<select class="rthd_system_emails" id="<?php echo esc_attr( $slug ); ?>_system_email"
				        name="<?php echo esc_attr( $slug ); ?>_system_email"
				        data-prev-val="<?php echo esc_attr( ( isset( $module_settings['system_email'] ) ) ? $module_settings['system_email'] : '' ); ?>">
			...
			?>
				<option
					value="<?php echo sanitize_email( $e->email ); ?>" <?php echo esc_attr( $selected ); ?>><?php echo sanitize_email( $e->email ); ?></option>
			<?php } ?>
				</select>
			</td>
			<th scope="row"><label
					for="<?php echo esc_attr( $slug ); ?>_outbound_emails"><?php echo esc_html( $rt_hd_module->name ). esc_attr( __( ' Outbound Emails' ) ); ?></label>
			</th>
			<td>
				<input type="text" id="<?php echo esc_attr( $slug ); ?>_outbound_emails" name="<?php echo esc_attr( $slug ); ?>_outbound_emails"
				       placeholder="Comma Separated Outbound Emails"
				       value="<?php echo esc_attr( ( isset( $module_settings['outbound_emails'] ) ) ? $module_settings['outbound_emails'] : '' ); ?>"/>
			</td>
		</tr>
		<script>
	...
	<?php } elseif ( $_REQUEST['type'] == 'other' ) { ?>
		...
		<tr valign="top">
			...
			<td><input type="text" name="rthd_menu_label" value="<?php echo esc_attr( rthd_get_menu_label() ); ?>"/></td>
		</tr>
		<tr valign="top">
			<th scope="row"><label for="rthd_logo_url"><?php _e( 'Helpdesk Plugin Icon (Logo) URL' ); ?></label></th>
			<td><input type="text" name="rthd_logo_url" value="<?php echo esc_url( rthd_get_logo_url() ); ?>"/></td>
		</tr>
	<?php
	}
} ?>
