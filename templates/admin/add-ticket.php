...
if ( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'trash' && isset( $_REQUEST[ $post_type . '_id' ] ) ) {
	...
	if ( ! $return ) {
		echo '<script> window.location="' . esc_url( admin_url( 'edit.php?post_type=' . $post_type . '&page=rthd-all-' . $post_type ) ) . '"; </script> ';
	}
}
...
if ( isset( $_REQUEST[ "{$post_type}_id" ] ) ) {
	...
	if ( isset( $_REQUEST['new'] ) ) {
		...
		<div class="alert-box success">
			<?php _e( 'New ' . esc_attr( ucfirst( $labels['name'] ) ) . ' Inserted Sucessfully.' ); ?>
	...
	if ( isset( $updateFlag ) && $updateFlag ) {
		?>
		<div class="alert-box success">
			<?php _e( esc_attr( ucfirst( $labels['name'] ) ) . ' Updated Sucessfully.' ); ?>
			<a href="#" class="close">&times;</a>
		</div>
...
<?php if ( $user_edit ) { ?>
	<form method='post' id="form-add-post" action="<?php echo esc_url( $form_ulr ); ?>">
	<?php } ?>
<?php if ( isset( $post->ID ) && $user_edit ) { ?>
	<input type="hidden" name="post[post_id]" id='ticket_id' value="<?php echo esc_attr( $post->ID ); ?>"/>
	<?php } ?>
...
?>
		<h4><i class="gen-enclosed <?php echo esc_attr( $post_icon ); ?>"></i> <?php _e( esc_attr( $page_title ) ); ?></h4>
	</div>
	<div class="large-5 columns rthd-sticky">
		...
		<?php } ?>
		<?php if ( $user_edit ) { ?>
			<button class="right mybutton success" type="submit"><?php _e( balanceTags( $save_button ) ); ?></button> &nbsp;&nbsp;
		<?php } ?>
		<?php if ( isset( $post->ID ) && $user_edit ) { ?>
...
<div class="large-6 small-12 columns ui-sortable meta-box-sortables">
	<div class="row collapse postbox">
		<div class="large-12 columns">
			...
			<?php } else { ?>
				<span><?php echo esc_attr( ( isset( $post->ID ) ) ? $post->post_title : '' ); ?></span><br/><br/>
			<?php } ?>
		</div>
	</div>
...
if ( $user_edit ) {
	...
	echo esc_attr( ucfirst( $labels['name'] ) ) . ' Content : <br /><br /><span>' . balanceTags( ( ( isset( $post->ID ) ) ? $post->post_content : '' ) . '</span><br /><br />' );
}
?>
		</div>
	</div>
...
if ( isset( $post->ID ) ) {
	...
	$post_users_meta = array(
		'subscribe_to',
		'ticket_closing_date',
		'ticket_external_file',
		'rthd_unique_id',
	);
			...
			<div class="row collapse">
				<div class="large-12 small-12 columns">
					<div class="large-4 small-4 columns">
						<?php if ( $user_edit ) { ?>
							<input type="text" name="postmeta[<?php echo esc_attr( $count ); ?>'][key]"
					...
					<div class="large-8 small-8 columns">
						<?php if ( $user_edit ) { ?>
							<input type="text" name="postmeta[<?php echo esc_attr( $count ); ?>'][value]"
							       value="<?php echo esc_attr( $pmeta ); ?>"/>
						<?php } else { ?>
							<span><?php echo esc_attr( $pmeta ); ?></span>
						<?php } ?>
					</div>
				</div>
			</div>
				...
				<div class="scroll-height">
					<?php foreach ( $attachments as $attachment ) { ?>
						...
						<div class="large-12 mobile-large-3 columns attachment-item"
							...
							<a target="_blank" href="<?php echo esc_url( wp_get_attachment_url( $attachment->ID ) ); ?>"> <img
			...
			<div class="large-12 small-12 columns">
				...
				<?php if ( $user_edit ) { ?>
					<div class="large-8 small-8 columns">
						<input type="text" name="ticket_ex_files[<?php echo esc_attr( $count ); ?>'][link]"
						       value="<?php echo esc_attr( $ex_file['link'] ); ?>"/>
					</div>
				...
				<?php } else { ?>
					<div class="large-9 small-9 columns">
						<span><?php echo esc_html( $pmeta ); ?></span>
					</div>
				<?php } ?>
			</div>
		</div>
...
<h6 class="hndle"><span><i
			class="foundicon-idea"></i> <?php _e( esc_attr( ucfirst( $labels['name'] ) ) . ' Information' ); ?></span></h6>
...
<div class="row collapse">
	...
	<div class="small-8 large-8 columns <?php echo esc_attr( ( ! $user_edit ) ? 'rthd_attr_border' : '' ); ?>">
		<?php
if ( isset( $post->ID ) ) {
...
?>
		...
		if ( $status['slug'] == $pstatus ) {
			...
			printf( '<option value="%s" %s >%s</option>', $status['slug'], $selected, $status['name'] );
				...
				<?php if ( $custom_status_flag && isset( $post->ID ) ) {
					echo '<option selected="selected" value="' . esc_attr( $pstatus ) . '">' . esc_html( $pstatus ). '</option>';
				} ?>
			</select>
		<?php
} else {
	foreach ( $post_status as $status ) {
		if ( $status['slug'] == $pstatus ) {
			echo '<span class="rthd_view_mode">' . esc_html( $status['name'] ) . '</span>';
			break;
		}
	}
} ?>
...
<div id="rthd_closing_reason_wrapper"
     class="row collapse <?php echo esc_attr( ( $pstatus === 'trash' ) ? 'show' : 'hide' ); ?> <?php echo esc_attr( ( ! $user_edit ) ? 'rthd_attr_border' : '' ); ?>">
	<div class="large-4 small-4 columns">
			<span class="prefix"
...
<div class="row collapse">
	...
	<div class="large-7 mobile-large-1 columns <?php echo esc_attr( ( ! $user_edit ) ? 'rthd_attr_border' : '' ); ?>">
		<?php if ( $user_edit ) { ?>
			...
			<input name="post[post_date]" type="hidden"
			       value="<?php echo esc_attr( ( isset( $createdate ) ) ? $createdate : '' ); ?>"/>
		<?php } else { ?>
			<span class="rthd_view_mode moment-from-now"><?php echo esc_html( $createdate ) ?></span>
		<?php } ?>
	</div>
...
<?php if ( isset( $post->ID ) ) { ?>
		...
		<div class="large-7 mobile-large-1 columns <?php echo esc_attr( ( ! $user_edit ) ? 'rthd_attr_border' : '' ); ?>">
			<?php if ( $user_edit ) { ?>
				<input class="moment-from-now" type="text" placeholder="Modified on Date"
				       value="<?php echo esc_attr( $modifydate ); ?>" title="<?php echo esc_attr( $modifydate ); ?>" readonly="readonly">
			<?php } else { ?>
				<span class="rthd_view_mode moment-from-now"><?php echo esc_html( $modifydate ); ?></span>
			<?php } ?>
		</div>
		<div class="large-1 mobile-large-1 columns">
				<span class="postfix datepicker-toggle" data-datepicker="closing-date"><label
	...
	<div class="row collapse">
		...
		<div class="large-7 mobile-large-2 columns <?php echo  esc_attr( ( ! $user_edit ) ? 'rthd_attr_border' : '' ); ?>">
			<?php if ( $user_edit ) { ?>
				...
				<input name="post[closing-date]" type="hidden"
				       value="<?php echo esc_attr( ( isset( $closingdate ) ) ? $closingdate : '' ); ?>"/>
			<?php } else { ?>
				<span class="rthd_view_mode moment-from-now"><?php echo esc_html( $closingdate ); ?></span>
			<?php } ?>
		</div>
		<?php if ( $user_edit ) { ?>
			<div class="large-1 mobile-large-1 columns">
		...
		<div class="row collapse">
			...
			<div class="large-8 mobile-large-3 columns">
				<div class="rthd_attr_border"><a class="rthd_public_link" target="_blank"
				                                 href="<?php echo esc_url( trailingslashit( site_url() ) . strtolower( $labels['name'] ) . '/?rthd_unique_id=' . $rthd_unique_id ); ?>"><?php _e( 'Link' ); ?></a>
				</div>
			</div>
		</div>
	<?php
...
<div class="row collapse postbox">
	...
	if ( isset( $selected_term_list ) && in_array( $term->term_id, $selected_term_list ) ) {
		$checked = 'checked';
				...
				<li><label for="user-group-<?php echo esc_attr( $key ); ?>"><input title=""
				                                                       id="user-group-<?php echo esc_attr( $key ); ?>" <?php echo esc_attr( $checked ); ?>
				                                                       name="post[user-group][]"
				                                                       value="<?php echo esc_attr( $term->slug ); ?>"
				                                                       type="checkbox"> <?php echo esc_html( $term->name ); ?>
					</label></li>
			<?php } ?>
		</ul>
	</div>
...
<?php foreach ( $attributes as $attr ) {
	...
	<div class="row collapse postbox">
		<div class="handlediv" title="<?php _e( 'Click to toggle' ); ?>"><br/></div>
		<h6 class="hndle"><span><i class="gen-enclosed foundicon-star"></i> <?php echo esc_attr( $attr->attribute_label ); ?></span>
		</h6>
...
<div class="large-3 columns ui-sortable meta-box-sortables">
	...
	if ( ! empty( $results ) ) {
		foreach ( $results as $author ) {
			if ( $author->ID == $post_author ) {
				echo '<div class="rthd_view_mode">' . get_avatar( $author->user_email, 17 ) . ' ' . esc_html( $author->display_name  ). '</div>';
				break;
			}
		}
	}
	...
	if ( isset( $post->ID ) ) {
		...
		foreach ( $ticket_term as $tterm ) {
			...
			echo "<li id='hd-account-" . esc_attr( $tterm->ID ). "' class='contact-list' >" . "<div class='row collapse'>" . "<div class='large-2 columns'> " . get_avatar( $email, 24 ) . '</div>' . "<div id='hd-account-meta-" . esc_attr( $tterm->ID ) . "'  class='large-9 columns'><a target='_blank' class='heading' href='" .esc_url( admin_url( 'edit.php?' . $account_post_type . '=' . $tterm->ID . '&post_type=' . $post_type ) ) . "' title='" . esc_attr( $tterm->post_title ). "'>" . esc_html( $tterm->post_title ) . '</a></div>' . "<div class='large-1 columns'><a href='#removeContact'><i class='foundicon-remove'></i></a><input type='hidden' name='post[accounts][]' value='" .esc_attr( $tterm->ID ). "' /></div>" . '</div>' . '</li>';
		}
	}
					?>
				</ul>
	...
	if ( isset( $post->ID ) ) {
		...
		foreach ( $ticket_term as $tterm ) {
			...
			echo "<li id='hd-contact-" . esc_attr( $tterm->ID ). "' class='contact-list' >" . "<div class='row collapse'>" . "<div class='large-2 columns'> " . get_avatar( $email, 24 ) . '</div>' . "<div id='hd-contact-meta-" . esc_attr( $tterm->ID ). "'  class='large-9 columns'><a target='_blank' class='heading' href='" . esc_url( admin_url( 'edit.php?' . rtbiz_post_type_name( 'contact' ) . '=' . esc_attr( $tterm->ID ). '&post_type=' . $post_type ) ) . "' title='" . esc_attr( $tterm->post_title ). "'>" . esc_html( $tterm->post_title  ). '</a>';
			if ( $email ) {
				echo "<i class=''><a href='mailto:" . sanitize_email( $email ) . "'>" . sanitize_email( $email ) . "</a><a class='inline' target='_blank' href='http://mail.google.com/mail/#search/" . sanitize_email( $email ) . "'> <i class='foundicon-search'></i></a></i>";
				$arrCommentReply[] = array(
					'userid'  => $tterm->ID,
					'label'   => $tterm->post_title,
					'email'   => $email,
					'contact' => true,
					'imghtml' => get_avatar( $email, 24 ),
				);
			}
			echo '</div>' . "<div class='large-1 columns'><a href='#removeContact'><i class='foundicon-remove'></i></a><input type='hidden' name='post[contacts][]' value='" . esc_attr( $tterm->ID  ). "' /></div>" . '</div>' . '</li>';
		}
	}
					?></ul>
			</div>
		...
		<div class="inside">
			<ul class="rthd-participant-list large-block-grid-1 small-block-grid-1">
				<?php foreach ( $all_hd_participants as $email ) {
					echo '<li>' . "<div class='row collapse'>" . "<div class='large-2 columns'> " . get_avatar( $email, 50 ) . '</div>' . "<div class='large-9 columns'><a target='_blank' class='heading' href='mailto:" . sanitize_email( $email ) . "' title='" . sanitize_email( $email ) . "'>" .  sanitize_email( $email ) . '</a></div>' . '</div>' . '</li>';
				} ?>
			</ul>
		</div>
	</div>
...
<?php if ( isset( $post->ID ) ) { ?>
	...
	$prev_year  = '';
	foreach ( $comments as $comment ) {
		rthd_get_template( 'admin/followup.php', array(
				'comment'   => $comment,
				'user_edit' => $user_edit,
			) );
								...
								<div class="row collapse">
									...
									<div class="large-10 mobile-large-2 columns">
										<select id="gm-from-email">
											<?php echo esc_html( $strAllowMailAc ); ?>
										</select>
									</div>
								</div>
								<div class="row collapse">
...
foreach ( $meta_fields as $field ) {
	...
	?>
	<option value="<?php echo esc_attr( $field['key'] ); ?>"><?php echo esc_attr( $field['text'] ); ?></option>
	<?php
}
?>
						</select>
