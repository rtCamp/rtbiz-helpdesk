...
if ( current_user_can( "edit_{$post_type}" ) ) {
	...
	if ( current_user_can( "read_{$post_type}" ) ) {
		$user_edit = 'false';
	} else {
		wp_die( "Opsss!! You are in restricted area" );
	}
...
if ( isset( $_POST[ 'post' ] ) ) {
	...
	$post = array( 'post_author' => $newTicket[ 'post_author' ], 'post_content' => $newTicket[ 'post_content' ], 'post_status' => $newTicket[ 'post_status' ], 'post_title' => $newTicket[ 'post_title' ], 'post_date' => $newTicket[ 'post_date' ], 'post_date_gmt' => $newTicket[ 'post_date_gmt' ], 'post_type' => $post_type );
	...
	if ( isset( $newTicket[ 'post_id' ] ) ) {
		...
		if ( $diff ) {
			...
			$id = $rt_hd_ticket_history_model->insert( array( 'ticket_id' => $newTicket[ 'post_id' ], 'type' => 'post_status', 'old_value' => $oldpost[ 'post_status' ], 'new_value' => $post[ 'post_status' ], 'update_time' => current_time( 'mysql' ), 'updated_by' => get_current_user_id(), ) );
		...
		/* Update Index Table */
		$data  = array( 'assignee' => $post[ 'post_author' ], 'post_content' => $post[ 'post_content' ], 'post_status' => $post[ 'post_status' ], 'post_title' => $post[ 'post_title' ], 'date_create' => $post[ 'post_date' ], 'date_create_gmt' => $post[ 'post_date_gmt' ], 'date_update' => current_time( 'mysql' ), 'date_update_gmt' => gmdate( 'Y-m-d H:i:s' ), 'user_updated_by' => get_current_user_id(), );
		...
		$data = array( 'post_id' => $post_id, 'assignee' => $post[ 'post_author' ], 'post_content' => $post[ 'post_content' ], 'post_status' => $post[ 'post_status' ], 'post_title' => $post[ 'post_title' ], 'date_create' => $post[ 'post_date' ], 'date_create_gmt' => $post[ 'post_date_gmt' ], 'date_update' => $post[ 'post_date' ], 'date_update_gmt' => $post[ 'post_date_gmt' ], 'user_updated_by' => get_current_user_id(), 'user_created_by' => get_current_user_id(), );
		$ticketModel->add_ticket( $data );
		/* System Notification - Ticket Inserted */

		/* Insert History for status */
		$id = $rt_hd_ticket_history_model->insert( array( 'ticket_id' => $post_id, 'type' => 'post_status', 'old_value' => '', 'new_value' => $post[ 'post_status' ], 'update_time' => current_time( 'mysql' ), 'updated_by' => get_current_user_id(), ) );
		if ( $post[ 'post_status' ] === 'closed' ) {
			$closing_reason_history_id = $id;
		}
	...
	if ( isset( $newTicket[ 'closing-date' ] ) && ! empty( $newTicket[ 'closing-date' ] ) ) {
		...
		update_post_meta( $post_id, '_rtbiz_hd_rtbiz_helpdesk_closing_date', $newTicket[ 'closing-date' ] );
		...
		$data      = array( 'date_closing' => gmdate( 'Y-m-d H:i:s', ( intval( $timeStamp ) + ( get_option( 'gmt_offset' ) * 3600 ) ) ), 'date_closing_gmt' => gmdate( 'Y-m-d H:i:s', ( intval( $timeStamp ) ) ), 'user_closed_by' => get_current_user_id(), 'date_update' => current_time( 'mysql' ), 'date_update_gmt' => gmdate( 'Y-m-d H:i:s' ), 'user_updated_by' => get_current_user_id(), );
		$where     = array( 'post_id' => $post_id );
		$ticketModel->update_ticket( $data, $where );
	...
	// Attachments
	$old_attachments = get_posts( array( 'post_parent' => $post_id, 'post_type' => 'attachment', 'fields' => 'ids', 'posts_per_page' => - 1, ) );
		...
		foreach ( $new_attachments as $attachment ) {
			if ( ! in_array( $attachment, $old_attachments ) ) {
				...
				if ( ! empty( $file->post_parent ) ) {
					$args = array( 'post_mime_type' => $file->post_mime_type, 'guid' => $file->guid, 'post_title' => $file->post_title, 'post_content' => $file->post_content, 'post_parent' => $post_id, 'post_author' => get_current_user_id(), );
	...
	if ( ! empty( $closing_reason_diff ) ) {
		...
		$data      = array( $attr_name => ( is_array( $attr_val ) ) ? implode( ',', $attr_val ) : $attr_val, 'date_update' => current_time( 'mysql' ), 'date_update_gmt' => gmdate( 'Y-m-d H:i:s' ), 'user_updated_by' => get_current_user_id(), );
		$where     = array( 'post_id' => $post_id );
		$ticketModel->update_ticket( $data, $where );
		...
		if ( $closing_reason_history_id ) {
			...
			$rt_hd_ticket_history_model->insert( array( 'ticket_id' => $post_id, 'type' => 'post_status', 'old_value' => $oldpost[ 'post_status' ], 'new_value' => $post[ 'post_status' ], 'message' => $message, 'update_time' => current_time( 'mysql' ), 'updated_by' => get_current_user_id(), ) );
		}
	}
	...
	if ( $module_settings[ 'attach_contacts' ] == 'yes' ) {
		...
		if ( ! empty( $contact_diff ) ) {
			...
			$data         = array( $contact_name => implode( ',', $contact_val ), 'date_update' => current_time( 'mysql' ), 'date_update_gmt' => gmdate( 'Y-m-d H:i:s' ), 'user_updated_by' => get_current_user_id(), );
			$where        = array( 'post_id' => $post_id );
			$ticketModel->update_ticket( $data, $where );
	...
	if ( $module_settings[ 'attach_accounts' ] == 'yes' ) {
		...
		if ( ! empty( $account_diff ) ) {
			...
			$data         = array( $account_name => implode( ',', $account_val ), 'date_update' => current_time( 'mysql' ), 'date_update_gmt' => gmdate( 'Y-m-d H:i:s' ), 'user_updated_by' => get_current_user_id(), );
			$where        = array( 'post_id' => $post_id );
			$ticketModel->update_ticket( $data, $where );
...
if ( isset( $_REQUEST[ "{$post_type}_id" ] ) ) {
	...
	if ( ! $post ) {
		...
		<div class="alert-box alert">
			Invalid Post ID <a href="" class="close">&times;</a>
		</div>
		<?php
		$post = false;
	}
	if ( $post->post_type != Rt_HD_Module::$post_type ) {
		?>
		<div class="alert-box alert">
			Invalid Post Type <a href="" class="close">&times;</a>
		</div>
		<?php
		$post = false;
	}
...
<div class="large-6 small-12 columns ui-sortable meta-box-sortables">
						...
						<div class="row collapse">
							<div class="large-12 small-12 columns">
								<div class="large-4 small-4 columns">
									<?php if ( $user_edit ) { ?>
										<input type="text" name="postmeta[<?php echo $count; ?>'][key]"
										       value="<?php echo $key; ?>"/>
									<?php } else { ?>
								...
								<div class="large-8 small-8 columns">
									<?php if ( $user_edit ) { ?>
										<input type="text" name="postmeta[<?php echo $count; ?>'][value]"
										       value="<?php echo $pmeta; ?>"/>
									<?php } else { ?>
	...
	if ( isset( $post->ID ) ) {
		$attachments = get_posts( array( 'posts_per_page' => - 1, 'post_parent' => $post->ID, 'post_type' => 'attachment', ) );
	}
	?>
	<div class="row collapse postbox">
		<div class="handlediv" title="<?php _e( 'Click to toggle' ); ?>"><br/></div>
				...
				<div class="scroll-height">
					<?php foreach ( $attachments as $attachment ) { ?>
						...
						<div class="large-12 mobile-large-3 columns attachment-item"
							...
							<a target="_blank" href="<?php echo wp_get_attachment_url( $attachment->ID ); ?>"> <img
			...
			if ( $user_edit ) {
				...
				<div class="row collapse" id="external-files-container">
					<div class="large-12 mobile-large-3 columns">
						...
						<div class="large-2 columns">
							<button id="add_new_ex_file" class="mybutton expand"
...
<div class="row collapse postbox">
<div class="handlediv" title="<?php _e( 'Click to toggle' ); ?>"><br/></div>
<h6 class="hndle"><span><i
			class="foundicon-idea"></i> <?php _e( ucfirst( $labels[ 'name' ] ) . " Information" ); ?></span></h6>
	...
	<div class="small-8 large-8 columns <?php echo ( ! $user_edit ) ? 'rthd_attr_border' : ''; ?>">
		...
		<?php if ( $user_edit ) { ?>
			<select id="rthd_post_status" class="right" name="post[post_status]">
				...
				<?php if ( $custom_status_flag && isset( $post->ID ) ) {
...
<div id="rthd_closing_reason_wrapper"
     class="row collapse <?php echo ( $pstatus === 'trash' ) ? 'show' : 'hide'; ?> <?php echo ( ! $user_edit ) ? 'rthd_attr_border' : ''; ?>">
	<div class="large-4 small-4 columns">
			<span class="prefix"
			      title="<?php _e( 'Closing Reason' ); ?>"><label><?php _e( 'Closing Reason' ); ?></label></span>
	</div>
	<div
		class="large-8 small-8 columns"><?php $rt_hd_closing_reason->get_closing_reasons( ( isset( $post->ID ) ) ? $post->ID : '', $user_edit ); ?></div>
</div>
<div class="row collapse">
	<div class="large-4 small-4 columns">
	...
	<div class="large-1 mobile-large-1 columns">
			<span class="postfix datepicker-toggle" data-datepicker="closing-date"><label
...
<?php if ( isset( $post->ID ) ) { ?>
	...
	<div class="row collapse">
		<div class="large-4 mobile-large-1 columns">
				<span class="prefix"
				      title="<?php _e( 'Closing Date' ); ?>"><label><?php _e( 'Closing Date' ); ?></label></span>
		</div>
...
if ( ! empty( $results ) ) {
	foreach ( $results as $author ) {
		if ( $get_assigned_to && ! empty( $get_assigned_to ) && in_array( $author->ID, $get_assigned_to ) ) {
			...
			$subScribetHTML .= "<li id='subscribe-auth-" . $author->ID . "' class='contact-list'>" . get_avatar( $author->user_email, 24 ) . '<a target="_blank" class="heading" title="' . $author->display_name . '" href="' . get_edit_user_link( $author->ID ) . '">' . $author->display_name . '</a>' . "<a class='right' href='#removeSubscriber'><i class='foundicon-remove'></i></a>
...
if ( isset( $post->ID ) ) {
	...
	if ( ! empty( $rthd_unique_id ) ) {
		...
		<div class="row collapse">
			<div class="large-4 mobile-large-1 columns">
					<span class="prefix"
					      title="<?php _e( 'Public URL' ); ?>"><label><?php _e( 'Public URL' ); ?></label></span>
			</div>
			<div class="large-8 mobile-large-3 columns">
				<div class="rthd_attr_border"><a class="rthd_public_link" target="_blank"
...
<?php } ?>
<?php if ( isset( $post->ID ) ) {
	do_action( 'rt_hd_after_ticket_information', $post, $user_edit );
} ?>
</div>
</div>
	...
	<div class="inside">
		<ul class="rt-form-checkbox scroll-height">
			<?php foreach ( $terms as $key => $term ) {
				...
				<li><label for="user-group-<?php echo $key; ?>"><input title=""
				                                                       id="user-group-<?php echo $key; ?>" <?php echo $checked; ?>
				                                                       name="post[user-group][]"
				                                                       value="<?php echo $term->slug; ?>"
				                                                       type="checkbox"> <?php echo $term->name; ?>
					</label></li>
			<?php } ?>
		</ul>
	</div>
...
<?php if ( isset( $post->ID ) ) {
	do_action( 'rt_hd_other_details', $user_edit, $post );
} ?>
<?php foreach ( $attributes as $attr ) {
	...
	<div class="row collapse postbox">
		<div class="handlediv" title="<?php _e( 'Click to toggle' ); ?>"><br/></div>
		<h6 class="hndle"><span><i class="gen-enclosed foundicon-star"></i> <?php echo $attr->attribute_label; ?></span>
		</h6>
			...
			<div class="inside">
				<?php if ( $user_edit ) { ?>
					<div class="row collapse">
						...
						<button class="large-3 mobile-large-1 columns mybutton" type="button" id="add-new-account">Add
						</button>
					</div>
				<?php } ?>
				<ul id="divAccountsList" class="block-grid large-1-up">
					...
					if ( isset( $post->ID ) ) {
						...
						foreach ( $ticket_term as $tterm ) {
							...
							echo "<li id='hd-account-" . $tterm->ID . "' class='contact-list' >" . "<div class='row collapse'>" . "<div class='large-2 columns'> " . get_avatar( $email, 24 ) . "</div>" . "<div id='hd-account-meta-" . $tterm->ID . "'  class='large-9 columns'><a target='_blank' class='heading' href='" . admin_url( "edit.php?" . $account_post_type . "=" . $tterm->ID . "&post_type=" . $post_type ) . "' title='" . $tterm->post_title . "'>" . $tterm->post_title . "</a></div>" . "<div class='large-1 columns'><a href='#removeContact'><i class='foundicon-remove'></i></a><input type='hidden' name='post[accounts][]' value='" . $tterm->ID . "' /></div>" . "</div>" . "</li>";
						}
					}
					?>
				</ul>
			...
			<div class="inside">
				<?php if ( $user_edit ) { ?>
					<div class="row collapse">
						...
						<button class="large-3 mobile-large-1 columns mybutton " type="button" id="add-new-contact">
							Add
						</button>
					</div>
				<?php } ?>
				<ul id="divContactsList" class="large-block-grid-1"><?php
					if ( isset( $post->ID ) ) {
						...
						foreach ( $ticket_term as $tterm ) {
							...
							echo "<li id='hd-contact-" . $tterm->ID . "' class='contact-list' >" . "<div class='row collapse'>" . "<div class='large-2 columns'> " . get_avatar( $email, 24 ) . "</div>" . "<div id='hd-contact-meta-" . $tterm->ID . "'  class='large-9 columns'><a target='_blank' class='heading' href='" . admin_url( "edit.php?" . rtbiz_post_type_name( 'contact' ) . "=" . $tterm->ID . "&post_type=" . $post_type ) . "' title='" . $tterm->post_title . "'>" . $tterm->post_title . "</a>";
							if ( $email ) {
							...
							echo "</div>" . "<div class='large-1 columns'><a href='#removeContact'><i class='foundicon-remove'></i></a><input type='hidden' name='post[contacts][]' value='" . $tterm->ID . "' /></div>" . "</div>" . "</li>";
						}
					}
					?></ul>
			</div>
		...
		<div class="inside">
			<ul class="rthd-participant-list large-block-grid-1 small-block-grid-1">
				<?php foreach ( $all_hd_participants as $email ) {
					echo "<li>" . "<div class='row collapse'>" . "<div class='large-2 columns'> " . get_avatar( $email, 50 ) . "</div>" . "<div class='large-9 columns'><a target='_blank' class='heading' href='mailto:" . $email . "' title='" . $email . "'>" . $email . "</a></div>" . "</div>" . "</li>";
				} ?>
			</ul>
		</div>
	</div>
		...
		<fieldset>
			...
			<div class="row">
				...
				<div class="large-8 column mobile-large-4">
					...
					<div class="row">
						<div class="large-3 columns">
							<div class="section-container vertical-nav" id="vertical-tab-header"
							     data-section="vertical-nav">
								<section class="section active">
									<p class="title"><a class="myfollowup" data-type="note"
									                    href="javascript:selecttab('note')" data-tab="note">Note</a></p>
								</section>
								<section class="section">
									<p class="title"><a class="myfollowup" data-type="mail"
									                    href="javascript:selecttab('mail')" data-tab="mail">Mail</a></p>
								</section>
								<section class="section">
									<p class="title"><a class="myfollowup" data-type="ac_import"
									                    href="javascript:selecttab('acim')" data-tab="acim">AC
											Importer</a></p>
								</section>
								<section class="section">
									<p class="title"><a class="myfollowup" data-type="gm_import"
									                    href="javascript:selecttab('gmim')" data-tab="gmim">Gmail
											Importer</a></p>
								</section>
							</div>
						</div>
						<div class="large-9 columns section vertical-tab-content">
							<div id="tabnote" class="content"></div>
							<div id="tabmail" class="content hide">
								<div class="row collapse">
									...
									<div class="large-6 mobile-large-2 columns">
										<label> <input type='checkbox' value='all' id='sendCommentMail'>Send Mail to
											Client Also</label>
								...
								<div class="row collapse email-ac">
									...
									<div class="large-11 mobile-large-2 columns">
										<input type="text" placeholder="Type Name" class="email-auto-complete"
										       id="comment-reply-to">
									</div>
				...
				<div class="large-4 column" id="followup-slider">
					<div class="row collapse">
						<div class="large-12 columns"><label for="followup-private"><input type="checkbox"
						                                                                   id="followup-private"/> <?php _e( 'Private ?' ); ?>
							</label></div>
			...
			<div class="row">
				<div class="large-12 columns right">
					<button class="mybutton add-savefollowup" id="savefollwoup"
					        type="button"><?php _e( "Add Followup" ); ?></button>
				</div>
			</div>
		</fieldset>
		...
		<fieldset>
			...
			<div class="row">
				...
				<div class="large-10 mobile-large-3 columns">
					<div class="row collapse">
						...
						<button class="large-3 mobile-large-1 columns mybutton" type="button"
						        id="contact-add-new-account"><i class="foundicon-plus"></i> Account
						</button>
					</div>
				</div>
			</div>
			<div class="row">
				<div class="large-2 mobile-large-1 columns"></div>
				<div class="large-10 mobile-large-3 columns">
					<div class="row collapse">
			...
			<fieldset>
				...
				<div class="row collapse">
					...
					<button class="large-1 mobile-large-1 columns mybutton" type="button" id="contact-add-new-meta"><i
							class="foundicon-plus"></i></button>
				...
				<div class="row collapse">
					...
					<div class="large-3 mobile-large-1 columns contact-meta">
						<select>
							...
							foreach ( $meta_fields as $field ) {
								if ( in_array( $field[ 'key' ], array( 'account_address', 'account_country' ) ) ) {
									continue;
								}
								?>
								<option value="<?php echo $field[ 'key' ]; ?>"><?php echo $field[ 'text' ]; ?></option>
					...
					<button class="large-1 mobile-large-1 columns mybutton" type="button" id="account-add-new-meta"><i
							class="foundicon-plus"></i></button>
