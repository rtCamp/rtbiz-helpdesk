...
if ( $_REQUEST['type'] == 'personal' ) {
	...
	if ( $google_client_id == '' || $google_client_secret == '' ) {
		echo '<tr valign="top"><td><div class="error"><p>Please set google api detail on Google API <a href="' . esc_url( admin_url( 'edit.php?post_type=' . Rt_HD_Module::$post_type . '&page=rthd-settings&type=googleApi&tab=admin-settings' ) ) . '">setting</a> page </p></div></td></tr>';
	...
	foreach ( $google_acs as $ac ) {
		...
		<tr>
			<td>
				<form method="post" autocomplete="off"
				      action="<?php echo esc_url( admin_url( 'edit.php?post_type=' . Rt_HD_Module::$post_type . '&page=rthd-settings&tab=my-settings&type=personal' ) ); ?>">
					<input type="hidden" name='mail_ac' value="<?php echo sanitize_email( $email ); ?>"/>
					<table class='hd-google-profile-table'>
						<tr valign="top">
							<th scope="row"><label><?php echo esc_attr( $personMarkup ); ?></label></th>
							<th scope="row"><label>Allow user</label></th>
							<td class="long"><input type='text' name='mail_acl_<?php echo esc_attr( $rCount ); ?>'
							                        class='rtcamp-user-ac' autocomplete="off"/>
		<?php
		$mail_users_acl = $rt_hd_settings->get_email_acl( $email );
		foreach ( $mail_users_acl as $acl ) {
			echo "<div class='mail-acl_user' acl-user='" . esc_attr( $acl->allow_user ). "' id='mail_acl_" . esc_attr( $rCount  ). '_' . esc_attr( $acl->allow_user ). "'>&nbsp;&nbsp;
				<a href='#removeAccess'>X</a><input type='hidden' name='allow_users[]' value='" . esc_attr( $acl->allow_user ) . "' /></div>";
		}
		?>
							</td>
						</tr>
						<tr valign="top">
							<td><?php if ( isset( $ac->email_data['name'] ) ) {
									echo esc_attr( $ac->email_data['name'] );
								} ?> <br/><a href='mailto:<?php echo sanitize_email( $email ) ?>'><?php echo sanitize_email( $email ) ?></a></td>
		...
		try {
			...
			echo '<tr valign="top"><td></td><td></td><td><p class="description">' . balanceTags( $e->getMessage() ) . '</p></td></tr>';
		}
						?>
						<tr valign="top">
							<td></td>
	...
	if ( $_REQUEST['type'] == 'activecollab' ) {
		...
		<tr valign="top">
			...
			<td><input type="text" name="rthd_activecollab_token"
			           value="<?php echo esc_attr( get_user_meta( get_current_user_id(), 'rthd_activecollab_token', true ) ); ?>"/>
			</td>
		</tr>
		<tr valign="top">
			...
			<td><input type="text" name="rthd_activecollab_default_project"
			           value="<?php echo esc_attr( get_user_meta( get_current_user_id(), 'rthd_activecollab_default_project', true ) ); ?>"/>
			</td>
		</tr>
	<?php
	}
...
<?php if ( $_REQUEST['type'] != 'personal' ) { ?>
	...
	<form id="rthd_add_imap_acc_form" autocomplete="off" class="rthd-hide-row" method="post"
	      action="<?php echo esc_url( admin_url( 'edit.php?post_type=' . Rt_HD_Module::$post_type . '&page=rthd-settings&type=personal&tab=my-settings' ) ); ?>">
