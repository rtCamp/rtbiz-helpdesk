	...
	<div id="rthd-followup-form" class="clearfix">
		<div class="rthd-attachment-box">
            ...
            <div>
						...
						<label
							for="followup_privacy_<?php echo Rt_HD_Import_Operation::$FOLLOWUP_SENSITIVE ?>"> Customers + Staff (<?php echo rt_biz_hd_get_comment_type( Rt_HD_Import_Operation::$FOLLOWUP_SENSITIVE ) ?>)
              ...
              <input type="radio" class="radio" name="private_comment"
						...
						<label
							for="followup_privacy_<?php echo Rt_HD_Import_Operation::$FOLLOWUP_STAFF ?>"> <?php echo rt_biz_hd_get_comment_type( Rt_HD_Import_Operation::$FOLLOWUP_STAFF ) ?> </label> <br />
            </div>
          </div>
				</div>
			<?php } else { ?>
				<label for="add-private-comment" class="description">
					...
					<?php echo 'Mark this as '.rt_biz_hd_get_comment_type( Rt_HD_Import_Operation::$FOLLOWUP_SENSITIVE )?></label>
					<span class="rthd-tooltip rthd-followup-type-tolltip">
						<i class="dashicons dashicons-info rtmicon"></i>
						<span class="rthd-tip">
							<?php _e( 'Email notification will not show content of this followup. Recommended, if you are sharing password or other sensitive information.' , RT_BIZ_HD_TEXT_DOMAIN ); ?>
			...
			<?php }	?>
			<?php if ( current_user_can( rt_biz_get_access_role_cap( RT_BIZ_HD_TEXT_DOMAIN, 'author' ) ) && $post->post_status != 'hd-answered' ) { ?>
