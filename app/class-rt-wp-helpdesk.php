...
if ( ! class_exists( 'RT_WP_Helpdesk' ) ) {
	...
	class RT_WP_Helpdesk {
		...
		function init_globals() {
			...
			global $rt_hd_mail_accounts_model, $rt_hd_mail_acl_model, $rt_hd_mail_thread_importer_model, $rt_hd_mail_message_model, $rt_hd_mail_outbound_model, $rt_hd_gravity_fields_mapping_model, $rt_hd_ticket_history_model, $rt_hd_imap_server_model, $rthd_form, $taxonomy_metadata, $rt_hd_reports, $rt_hd_closing_reason, $rt_hd_attributes, $rt_hd_dashboard, $rt_hd_module, $rt_hd_acl, $rt_hd_accounts, $rt_hd_contacts, $rt_hd_tickets, $rt_hd_gravity_form_importer, $rt_hd_gravity_form_mapper, $rt_hd_settings, $rt_hd_user_settings, $rt_hd_logs, $rt_hd_woocommerce;
		 ...
		 * @since rt-Helpdesk 0.1
		 */
		function loadScripts( $rthd_unique_id = null ) {
			global $wp_query;
		...
		function localize_scripts( $rthd_unique_id = null) {
			...
			$args       = array( 'meta_key' => '_rtbiz_hd_unique_id', 'meta_value' => $unique_id, 'post_status' => 'any', 'post_type' => Rt_HD_Module::$post_type, );
			$ticketpost = get_posts( $args );
			if ( empty( $ticketpost ) ) {
				return;
			}
