	...
	if ( isset( $google_acs ) && ! empty( $google_acs ) ) {
		foreach ( $google_acs as $ac ) {
			...
			<table class="form-table hd-option">
				...
				<tr>
					<td>
						<input type="hidden" name='mail_ac' value="<?php echo sanitize_email( $email ); ?>"/>
						<table class='hd-google-profile-table'>
							<?php if ( $ac->type == 'imap' ) { ?>
								<tr valign="top">
									<td></td>
									<th scope="row"><label><?php _e( 'IMAP Server', RT_HD_TEXT_DOMAIN  ); ?></label></th>
									<td class="long">
										<select required="required" name="imap_server">
											<option value=""><?php _e( 'Select Mail Server', RT_HD_TEXT_DOMAIN  ); ?></option>
											<?php $imap_servers = $rt_hd_imap_server_model->get_all_servers();
				foreach ( $imap_servers as $server ) {
					?>
					<option <?php echo esc_attr( (  isset( $ac->imap_server ) && $ac->imap_server == $server->id ) ? 'selected="selected"' : '' ); ?>
						value="<?php echo esc_attr( $server->id ); ?>"><?php echo esc_attr( $server->server_name ); ?></option>
				<?php } ?>
										</select>
									</td>
								</tr>
								<tr valign="top">
									<td></td>
									<th scope="row"><label><?php _e( 'Password', RT_HD_TEXT_DOMAIN  ); ?></label></th>
									<td class="long"><input required="required" autocomplete="off" type="password"
									                        name="imap_password" placeholder="Password"
									                        value="<?php echo esc_attr( rthd_encrypt_decrypt( $token ) ); ?>"/></td>
								</tr>
			<?php
			}
			$all_folders = null;
			try {
				...
				echo '<tr valign="top"><td></td><td></td><td><p class="description">' . esc_attr( $e->getMessage() ) . '</p></td></tr>';
			} ?>
							<tr valign="top">
								<td><label><?php echo balanceTags( $personMarkup ); ?></label></td>
								<th scope="row"><label><?php _e( 'Mail Folders to read', RT_HD_TEXT_DOMAIN  ); ?></label></th>
								<td>
									<label> <?php _e( 'Inbox Folder', RT_HD_TEXT_DOMAIN  ); ?>
										<select data-email-id="<?php echo esc_attr( $ac->id ); ?>" name="inbox_folder"
										        data-prev-value="<?php echo esc_attr( $inbox_folder ); ?>">
											<option value=""><?php _e( 'Choose Inbox Folder', RT_HD_TEXT_DOMAIN  ); ?></option>
									...
									<?php if ( in_array( $email, rthd_get_all_system_emails() ) ) { ?>
										<p class="description"><?php _e( 'This is linked as a system mail. Hence it will only read the Inbox Folder; no matter what folder you choose over here. These will be ignored.', RT_HD_TEXT_DOMAIN  ); ?></p>
									<?php } ?>
			<?php if ( ! is_null( $all_folders ) ) {
				...
				echo '<p class="description">' . esc_attr( __( 'No Folders found.' ) ) . '</p>';
			} ?>
								</td>
							</tr>
							<tr valign="top">
		...
		foreach ( $imap_servers as $server ) {
			?>
			<option value="<?php echo esc_attr( $server->id ); ?>"><?php echo esc_attr( $server->server_name ); ?></option>
		<?php } ?>
			</select>
...
function rthd_imap_servers( $field, $value ) {
	...
	<table>
		...
		<?php foreach ( $servers as $server ) { ?>
			<tr valign="top">
				<th scope="row"><?php echo esc_attr( $server->server_name ); ?></th>
				<td>
			...
			<tr valign="top" id="rthd_imap_server_<?php echo esc_attr( $server->id ); ?>" class="rthd-hide-row">
				<td>
					<table>
						...
						<tr valign="top">
							...
							<td>
								<select name="rthd_imap_servers[<?php echo esc_attr( $server->id ); ?>][incoming_imap_enc]">
									<option value=""><?php  _e( 'Select Encryption Method', RT_HD_TEXT_DOMAIN ); ?></option>
									<option
						...
						<tr valign="top">
							<th scope="row"><?php _e( 'SMTP (Outgoing) Server: ', RT_HD_TEXT_DOMAIN  ); ?></th>
							<td><input type="text" required="required"
						...
						<tr valign="top">
							<th scope="row"><?php _e( 'SMTP (Outgoing) Port: ', RT_HD_TEXT_DOMAIN  ); ?></th>
							<td><input type="text" required="required"
						...
						<tr valign="top">
							<th scope="row"><?php _e( 'SMTP (Outgoing) Encryption: ', RT_HD_TEXT_DOMAIN ); ?></th>
							<td>
		...
		<tr valign="top">
			<th scope="row"><a href="#" class="button" id="rthd_add_imap_server"><?php _e( 'Add new server', RT_HD_TEXT_DOMAIN  ); ?></a>
			</th>
		</tr>
		<tr valign="top" id="rthd_new_imap_server" class="rthd-hide-row">
			<td>
				<table>
					<tr valign="top">
						<th scope="row"><?php _e( 'Server Name: ', RT_HD_TEXT_DOMAIN  ); ?></th>
						<td><input type="text" name="rthd_imap_servers[new][server_name]"/></td>
					</tr>
					<tr valign="top">
						<th scope="row"><?php _e( 'IMAP (Incoming) Server: ', RT_HD_TEXT_DOMAIN  ); ?></th>
						<td><input type="text" name="rthd_imap_servers[new][incoming_imap_server]"/></td>
					</tr>
					<tr valign="top">
						<th scope="row"><?php _e( 'IMAP (Incoming) Port: ', RT_HD_TEXT_DOMAIN  ); ?></th>
						<td><input type="text" name="rthd_imap_servers[new][incoming_imap_port]"/></td>
					</tr>
					<tr valign="top">
						<th scope="row"><?php _e( 'IMAP (Incoming) Encryption: ', RT_HD_TEXT_DOMAIN  ); ?></th>
						<td>
							<select name="rthd_imap_servers[new][incoming_imap_enc]">
								<option value=""><?php _e( 'Select Encryption Method', RT_HD_TEXT_DOMAIN  ); ?></option>
								<option value="ssl"><?php _e( 'SSL', RT_HD_TEXT_DOMAIN  ); ?></option>
								<option value="tls"><?php _e( 'TLS', RT_HD_TEXT_DOMAIN  ); ?></option>
							</select>
						</td>
					</tr>
					<tr valign="top">
					...
					<tr valign="top">
						<th scope="row"><?php _e( 'Is SSL required for SMTP (Outgoing Mails): ', RT_HD_TEXT_DOMAIN  ); ?></th>
						<td>
