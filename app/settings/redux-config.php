...
if ( ! class_exists( 'Redux_Framework_Helpdesk_Config' ) ) {
	...
	class Redux_Framework_Helpdesk_Config {
				...
				if ( isset( $_POST[ 'rthd_imap_servers' ] ) ) {
					...
					foreach ( $old_servers as $id => $server ) {
						if ( isset( $new_servers[ $server->id ] ) ) {
							...
							$args = array( 'server_name' => $new_servers[ $server->id ][ 'server_name' ], 'incoming_imap_server' => $new_servers[ $server->id ][ 'incoming_imap_server' ], 'incoming_imap_port' => $new_servers[ $server->id ][ 'incoming_imap_port' ], 'incoming_imap_enc' => ( isset( $new_servers[ $server->id ][ 'incoming_imap_enc' ] ) && ! empty( $new_servers[ $server->id ][ 'incoming_imap_enc' ] ) ) ? $new_servers[ $server->id ][ 'incoming_imap_enc' ] : null, 'outgoing_smtp_server' => $new_servers[ $server->id ][ 'outgoing_smtp_server' ], 'outgoing_smtp_port' => $new_servers[ $server->id ][ 'outgoing_smtp_port' ], 'outgoing_smtp_enc' => ( isset( $new_servers[ $server->id ][ 'outgoing_smtp_enc' ] ) && ! empty( $new_servers[ $server->id ][ 'outgoing_smtp_enc' ] ) ) ? $new_servers[ $server->id ][ 'outgoing_smtp_enc' ] : null, );
					...
					if ( ! empty( $new_servers[ 'new' ][ 'server_name' ] ) && ! empty( $new_servers[ 'new' ][ 'incoming_imap_server' ] ) && ! empty( $new_servers[ 'new' ][ 'incoming_imap_port' ] ) && ! empty( $new_servers[ 'new' ][ 'outgoing_smtp_server' ] ) && ! empty( $new_servers[ 'new' ][ 'outgoing_smtp_port' ] ) ) {
						...
						$args = array( 'server_name' => $new_servers[ 'new' ][ 'server_name' ], 'incoming_imap_server' => $new_servers[ 'new' ][ 'incoming_imap_server' ], 'incoming_imap_port' => $new_servers[ 'new' ][ 'incoming_imap_port' ], 'incoming_imap_enc' => ( isset( $new_servers[ $server->id ][ 'incoming_imap_enc' ] ) && ! empty( $new_servers[ $server->id ][ 'incoming_imap_enc' ] ) ) ? $new_servers[ $server->id ][ 'incoming_imap_enc' ] : null, 'outgoing_smtp_server' => $new_servers[ 'new' ][ 'outgoing_smtp_server' ], 'outgoing_smtp_port' => $new_servers[ 'new' ][ 'outgoing_smtp_port' ], 'outgoing_smtp_enc' => ( isset( $new_servers[ $server->id ][ 'outgoing_smtp_enc' ] ) && ! empty( $new_servers[ $server->id ][ 'outgoing_smtp_enc' ] ) ) ? $new_servers[ $server->id ][ 'outgoing_smtp_enc' ] : null, );
		...
		function dynamic_section( $sections ) {
			//$sections = array();
			$sections[ ] = array( 'title'  => __( 'Section via hook', 'redux-framework-demo' ), 'desc' => __( '<p class="description">This is a section created by adding a filter to the sections array. Can be used by child themes to add/remove sections from the options.</p>', 'redux-framework-demo' ), 'icon' => 'el-icon-paper-clip', // Leave this as a blank section, no options just some intro text set above.
			                      'fields' => array() );
		...
		public function setSections() {
			...
			$this->sections[ ] = array( 'icon' => 'el-icon-cogs', 'title' => __( 'General' ), 'permissions' => $admin_cap, 'fields' => array( array( 'id' => 'rthd_menu_label', 'type' => 'text', 'title' => __( 'Menu Label' ), 'subtitle' => __( 'Menu Label Identity for the Plugin.' ), 'desc' => __( 'This label will be used for the Menu Item label for Helpdesk' ), 'default' => __( 'rtHelpdesk' ), 'placeholder' => __( 'rtHelpdesk, Helpdesk, etc.' ), ), array( 'id' => 'rthd_logo_url', 'type' => 'media', 'url' => true, 'title' => __( 'Logo' ), 'desc' => __( 'This logo will be used for all the Menu, Submenu, Post Types Menu Icons in Helpdesk.' ), 'subtitle' => __( 'Upload any logo using the WordPress native uploader, preferrably with the size of 16x16.' ), 'default' => array( 'url' => RT_HD_URL . 'app/assets/img/hd-16X16.png', ), ), array( 'id' => 'rthd_support_page', 'type' => 'select', 'data' => 'pages', 'title' => __( 'Support Page' ), 'desc' => __( 'This Page will used for redirect support request in WooCommerce.' ), 'subtitle' => __( 'Select Page for Product Support' ),

			), array( 'id' => 'rthd_default_user', 'type' => 'select', 'options' => $users_options, 'default' => '1', 'title' => __( 'Default Assignee' ), 'desc' => __( 'Default User for HelpDesh ticket Assignee' ), 'subtitle' => __( 'Select User for Support ticket Assign' ), ), ), );
			...
			$this->sections[ ] = array( 'icon' => 'el-icon-envelope', 'title' => __( 'Mail Setup' ), 'permissions' => $admin_cap, 'fields' => array( array( 'id' => 'rthd_outgoing_email_from_address', 'title' => __( 'Outgoing Emails\' FROM Address' ), 'subtitle' => __( 'Outgoing System Email used for all Helpdesk Communication' ), 'desc' => sprintf( '%s <a href="%s">%s</a>. %s.', __( 'WordPress by default sends email using (mostly postfix) FROM/TO value set to Admin Email taken from' ), admin_url( 'options-general.php' ), __( 'here' ), __( 'System Email Address to be used for outbound emails. This Address will be used as FROM: email address for all outgoing emails' ) ), 'type' => 'text', 'default' => get_option( 'admin_email' ), 'validate' => 'email', ), array( 'id' => 'rthd_outgoing_email_delivery', 'title' => __( 'Outgoing Emails\' Delivery' ), 'subtitle' => __( 'This is how the emails will be sent from the Helpdesk system.' ), 'desc' => __( '' ), 'type' => 'radio', 'options' => array( 'wp_mail' => __( 'WordPress wp_mail Function' ), 'user_mail_login' => __( 'User\'s own Mail Login - (Google OAuth / SMTP Login etc. as per configuration)' ), 'amazon_ses' => __( 'Amazon SES - (Articles) - Not working as of now' ), 'google_smtp' => __( 'Google SMTP - NOT Recommended (Articles) - Not working as of now' ) ), 'default' => 'wp_mail', ), array( 'id' => 'rthd_notification_emails', 'title' => __( 'Notification Emails' ), 'subtitle' => __( 'Email addresses to be notified on events' ), 'desc' => __( 'These email addresses will be notified of the events that occurs in HelpDesk Systems. This is a global list. All the subscribers also will be notified along with this list.' ), 'type' => 'multi_text', 'validate' => 'email', 'multi' => true, ), array( 'id' => 'rthd_notification_events', 'title' => __( 'Notification Events' ), 'subtitle' => __( 'Events to be notified to users' ), 'desc' => __( 'These events will be notified to the Notification Emails whenever they occur.' ), 'type' => 'checkbox', 'options' => array( 'new_ticket_created' => __( 'Whenever a New Ticket is created.' ), 'new_comment_added' => __( 'Whenever a New Comment is added to a Ticket.' ), 'status_metadata_changed' => __( 'Whenever any status or metadata changed for a Ticket.' ), ), ), array( 'id' => 'rthd_enable_reply_by_email', 'type' => 'switch', 'title' => __( 'Enable Reply by Email' ), 'subtitle' => __( 'This feature' ), 'default' => false, 'on' => __( 'Enable' ), 'off' => __( 'Disable' ), ), array( 'id'       => 'section-media-start', 'type' => 'section', 'indent' => true, // Indent all options below until the next 'section' option is set.
			                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        'required' => array( 'rthd_enable_reply_by_email', "=", 1 ), ), array( 'id' => 'rthd_reply_by_email_view', 'type' => 'callback', 'callback' => 'rthd_reply_by_email_view', ), ), );

			$this->sections[ ] = array( 'icon' => 'el-icon-googleplus', 'title' => __( 'Google OAuth' ), 'permissions' => $admin_cap, 'subsection' => true, 'fields' => array( array( 'id' => 'rthd_googleapi_clientid', 'type' => 'text', 'title' => __( 'Google API Client ID' ), 'subtitle' => __( 'Subtitle' ), 'desc' => sprintf( '<p class="description">%s <a href="https://console.developers.google.com">%s</a>, %s <b>%s</b></p>', __( 'Create an app on' ), __( 'Google API Console' ), __( 'set authorized redirect urls to' ), $redirect_url ), ), array( 'id' => 'rthd_googleapi_clientsecret', 'type' => 'text', 'title' => __( 'Google API Client Secret' ), 'subtitle' => __( 'Subtitle' ), ), ), );

			$this->sections[ ] = array( 'icon' => 'el-icon-network', 'title' => __( 'IMAP Servers' ), 'permissions' => $admin_cap, 'subsection' => true, 'fields' => array( array( 'id' => 'rthd_imap_servers', 'type' => 'callback', 'title' => __( 'IMAP Servers' ), 'subtitle' => __( 'This section lets you configure different IMAP & SMTP Mail Servers e.g., Outlook, Google, Yahoo etc.' ), 'callback' => 'rthd_imap_servers', ), ), );

			ob_start();
			rthd_ticket_impoters();
			$importers_content = ob_get_clean();
			$this->sections[ ] = array( 'title'  => __( 'Importers' ), 'desc' => __( 'This section lets you import tickets into Helpdesk System from either a CSV file or any other Form Manager Plugin e.g., Gravity Forms.' ), 'icon' => 'el-icon-upload', 'permissions' => $editor_cap, //'subsection' => true,
			                            'fields' => array( array( 'id' => 'rthd_ticket_importers', 'type' => 'raw', 'content' => $importers_content, ), ), );

			ob_start();
			rthd_ticket_import_mapper();
			$import_mapper_content = ob_get_clean();
			$this->sections[ ]     = array( 'title' => __( 'Mapper' ), 'desc' => __( 'This section lets you view all the Import Mappings existing for ticket importing into Helpdesk System.' ), 'icon' => 'el-icon-map-marker', 'permissions' => $editor_cap, 'subsection' => true, 'fields' => array( array( 'id' => 'rthd_ticket_import_mapper', 'type' => 'raw', 'content' => $import_mapper_content, ), ), );

			ob_start();
			rthd_ticket_import_logs();
			$import_log_content = ob_get_clean();
			$this->sections[ ]  = array( 'title' => __( 'Import Logs' ), 'icon' => 'el-icon-list-alt', 'permissions' => $editor_cap, 'subsection' => true, 'fields' => array( array( 'id' => 'rthd_ticket_import_logs', 'type' => 'raw', 'content' => $import_log_content, ), ), );

			$this->sections[ ] = array( 'title' => __( 'Miscellaneous' ), 'heading' => __( 'Import / Export Settings' ), 'desc' => __( 'Import and Export your settings from file, text or URL.' ), 'icon' => 'el-icon-refresh', 'fields' => array( array( 'id' => 'rthd_settings_import_export', 'type' => 'import_export', 'title' => __( 'Import Export' ), 'subtitle' => 'Save and restore your options', 'full_width' => false, ), ), );
		}

		public function setHelpTabs() {

			// Custom page help tabs, displayed using the help API. Tabs are shown in order of definition.
			$this->args[ 'help_tabs' ][ ] = array( 'id' => 'redux-help-tab-1', 'title' => __( 'Theme Information 1', 'redux-framework-demo' ), 'content' => __( '<p>This is the tab content, HTML is allowed.</p>', 'redux-framework-demo' ) );

			$this->args[ 'help_tabs' ][ ] = array( 'id' => 'redux-help-tab-2', 'title' => __( 'Theme Information 2', 'redux-framework-demo' ), 'content' => __( '<p>This is the tab content, HTML is allowed.</p>', 'redux-framework-demo' ) );
		...
		public function setArguments() {
			...
			$this->args = array( // TYPICAL -> Change these values as you need/desire
			                     ...
			                     'menu_title'         => __( 'Settings' ), 'page_title' => __( 'Settings' ), // You will need to generate a Google API key to use this feature.
			...
			$this->args[ 'share_icons' ][ ] = array( 'url' => 'https://github.com/ReduxFramework/ReduxFramework', 'title' => 'Visit us on GitHub', 'icon' => 'el-icon-github' //'img'   => '', // You can use icon OR img. IMG needs to be a full URL.
			);
			...
			$this->args[ 'share_icons' ][ ] = array( 'url' => 'http://twitter.com/reduxframework', 'title' => 'Follow us on Twitter', 'icon' => 'el-icon-twitter' );
			$this->args[ 'share_icons' ][ ] = array( 'url' => 'http://www.linkedin.com/company/redux-framework', 'title' => 'Find us on LinkedIn', 'icon' => 'el-icon-linkedin' );
...
function rthd_reply_by_email_view( $field, $value ) {
	...
	if ( ( $google_client_id == "" || $google_client_secret == "" ) ) {
		echo '<div id="error_handle" class="error"><p>Please set google api detail on Google API <a href="' . admin_url( "edit.php?post_type=" . Rt_HD_Module::$post_type . "&page=rthd-settings&type=googleApi&tab=admin-settings" ) . '">setting</a> page </p></div>';
							...
							<?php if ( $ac->type == 'imap' ) { ?>
									...
									<td class="long">
										<select required="required" name="imap_server">
											...
											foreach ( $imap_servers as $server ) {
												?>
												<option <?php echo ( isset( $ac->imap_server ) && $ac->imap_server == $server->id ) ? 'selected="selected"' : ''; ?>
													value="<?php echo $server->id; ?>"><?php echo $server->server_name; ?></option>
											<?php } ?>
										</select>
									</td>
								</tr>
								<tr valign="top">
									...
									<td class="long"><input required="required" autocomplete="off" type="password"
									                        ...
									                        value="<?php echo rthd_encrypt_decrypt( $token ); ?>"/></td>
								</tr>
							<?php
							}
							$all_folders = null;
							...
							<tr valign="top">
								...
								<td>
									<input type="hidden" name="rthd_submit_enable_reply_by_email" value="save"/> <a
										...
										href='<?php echo admin_url( "edit.php?post_type=" . Rt_HD_Module::$post_type . "&page=rthd-settings&tab=my-settings&type=personal&email=" . $email ); ?>'>Remove
										A/C</a>
									<?php if ( $ac->type == 'goauth' ) { ?>
										<a class='button button-primary' href='<?php echo $authUrl; ?>'>Re Connect
											Google Now</a>
									<?php } ?>
								</td>
							</tr>
			...
			<tr valign="top">
				...
				<td>
					<a href="#" class="rthd-edit-server"
					   data-server-id="<?php echo $server->id; ?>"><?php _e( 'Edit' ); ?></a> <a href="#"
					                                                                             class="rthd-remove-server"
					                                                                             data-server-id="<?php echo $server->id; ?>"><?php _e( 'Remove' ); ?></a>
				</td>
			</tr>
					...
					<table>
						<tr valign="top">
							...
							<td><input type="text" required="required"
							           name="rthd_imap_servers[<?php echo $server->id; ?>][server_name]"
							           value="<?php echo $server->server_name; ?>"/></td>
						</tr>
						<tr valign="top">
							<th scope="row"><?php _e( 'IMAP (Incoming) Server: ' ); ?></th>
							<td><input type="text" required="required"
							           name="rthd_imap_servers[<?php echo $server->id; ?>][incoming_imap_server]"
							           value="<?php echo $server->incoming_imap_server; ?>"/></td>
						</tr>
						<tr valign="top">
							<th scope="row"><?php _e( 'IMAP (Incoming) Port: ' ); ?></th>
							<td><input type="text" required="required"
							           name="rthd_imap_servers[<?php echo $server->id; ?>][incoming_imap_port]"
							           value="<?php echo $server->incoming_imap_port; ?>"/></td>
						</tr>
						<tr valign="top">
							...
							<td>
								<select name="rthd_imap_servers[<?php echo $server->id; ?>][incoming_imap_enc]">
									...
									<option
										value="ssl" <?php echo ( $server->incoming_imap_enc == 'ssl' ) ? 'selected="selected"' : ''; ?>><?php _e( 'SSL' ); ?></option>
									<option
										value="tls" <?php echo ( $server->incoming_imap_enc == 'tls' ) ? 'selected="selected"' : ''; ?>><?php _e( 'TLS' ); ?></option>
								</select>
							</td>
						</tr>
						<tr valign="top">
							...
							<td><input type="text" required="required"
							           name="rthd_imap_servers[<?php echo $server->id; ?>][outgoing_smtp_server]"
							           value="<?php echo $server->outgoing_smtp_server; ?>"/></td>
						</tr>
						<tr valign="top">
							<th scope="row"><?php _e( 'SMTP (Outgoing) Port: ' ); ?></th>
							<td><input type="text" required="required"
							           name="rthd_imap_servers[<?php echo $server->id; ?>][outgoing_smtp_port]"
							           value="<?php echo $server->outgoing_smtp_port; ?>"/></td>
						</tr>
						<tr valign="top">
							...
							<td>
								<select name="rthd_imap_servers[<?php echo $server->id; ?>][outgoing_smtp_enc]">
									<option value=""><?php _e( 'Select Encryption Method' ); ?></option>
									<option
										value="ssl" <?php echo ( $server->outgoing_smtp_enc == 'ssl' ) ? 'selected="selected"' : ''; ?>><?php _e( 'SSL' ); ?></option>
									<option
										value="tls" <?php echo ( $server->outgoing_smtp_enc == 'tls' ) ? 'selected="selected"' : ''; ?>><?php _e( 'TLS' ); ?></option>
								</select>
							</td>
						</tr>
					</table>
