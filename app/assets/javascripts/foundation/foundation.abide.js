...
(function ( $, window, document, undefined ) {
	...
	Foundation.libs.abide = {
		...
		validate: function ( els, e ) {
			var validations = this.parse_patterns( els ), validation_count = validations.length, form = $( els[0] ).closest( 'form' );
		...
		check_validation_and_apply_styles: function ( el_patterns ) {
			...
			for ( var i = count - 1; i >= 0; i -- ) {
				var el = el_patterns[i][0], required = el_patterns[i][2], value = el.value, is_radio = el.type === "radio", valid_length = (required) ? (el.value.length > 0) : true;
