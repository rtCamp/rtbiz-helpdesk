...
(function ( $ ) {
	var w = window, doc = document, maxTopPos, minTopPos, pastStartOffset, objFartherThanTopPos, objBiggerThanWindow, newpos,
	...
	Sticky.prototype = {
		...
		rePosition: function ( quick, force ) {
			var $obj = this.obj, settings = this.settings, duration = quick ? 0 : settings.duration, wScroll = w.pageYOffset || doc.documentElement.scrollTop, wHeight = w.innerHeight || doc.documentElement.offsetHeight, objHeight = $obj[0].clientHeight;

			$obj.stop(); // stop jquery animation on scroll event

			if ( settings.lockBottom ) {
				maxTopPos = $obj[0].parentNode.clientHeight - objHeight - settings.offsetBottom;
			} // get the maximum top position of the floated element inside it's parent
			...
			if ( ((pastStartOffset || objFartherThanTopPos) && objBiggerThanWindow) || force ) {
				newpos = settings.stickToBottom ? wScroll + wHeight - objHeight - settings.startOffset - settings.offsetY : wScroll - settings.startOffset + settings.offsetY;
	...
	function getComputed( $obj ) {
		var p = $obj.parent(), ob = parseInt( p.css( 'padding-bottom' ) ), oy = parseInt( p.css( 'padding-top' ) ), so = p.offset().top;
