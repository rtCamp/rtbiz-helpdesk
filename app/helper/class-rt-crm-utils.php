...
if( !class_exists( 'Rt_CRM_Utils' ) ) {
	class Rt_CRM_Utils {
		...
		static public function setAccounts($rCount){
			$log_file = RT_CRM_PATH . "mailaccount.txt";
		...
		public static function get_mime_type($file) {
			...
			if(isset(Rt_CRM_Utils::$mime_types[$extension]))
				return Rt_CRM_Utils::$mime_types[$extension];
			else {
		...
		public static function get_extention($file){
			...
			foreach(Rt_CRM_Utils::$mime_types as $key=>$mime){
				if($mime== $file)
					return $key;
			}
			return "tmp";
