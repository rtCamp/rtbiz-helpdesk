...
if ( ! class_exists( 'Rt_HD_Tickets' ) ) {
	...
	class Rt_HD_Tickets {
		...
		public function insert_new_ticket( $title, $body, $userid, $mailtime, $allemail, $uploaded, $senderEmail, $messageid = "", $inreplyto = "", $references = "", $subscriber = array() ) {
			...
			$post_date     = gmdate( 'Y-m-d H:i:s', ( intval( $timeStamp ) ) );
			...
			$post    = array( 'post_author' => $userid, 'post_content' => $body, 'post_date' => $post_date, 'post_status' => 'unanswered', 'post_title' => $title, 'post_type' => $post_type, 'post_date_gmt' => $post_date_gmt, );
			...
			$data = array( 'post_id' => $post_id, 'assignee' => $post[ 'post_author' ], 'post_content' => $post[ 'post_content' ], 'post_status' => $post[ 'post_status' ], 'post_title' => $post[ 'post_title' ], 'date_create' => $post[ 'post_date' ], 'date_create_gmt' => $post[ 'post_date_gmt' ], 'date_update' => current_time( 'mysql' ), 'date_update_gmt' => gmdate( 'Y-m-d H:i:s' ), 'user_updated_by' => get_current_user_id(), 'user_created_by' => get_current_user_id(), );
		...
		function add_attachment_to_post( $uploaded, $post_id, $post_type, $mainTicket = true, $comment_id = 0 ) {
			if ( isset( $uploaded ) && is_array( $uploaded ) ) {
				...
				foreach ( $uploaded as $upload ) {
					...
					//$uploaded["filename"]
					$attachment = array( 'post_title' => $upload[ "filename" ], 'image_alt' => $upload[ "filename" ], 'post_type' => $post_type, 'post_content' => "", 'post_excerpt' => "", 'post_parent' => $post_id, 'post_mime_type' => $this->get_mime_type_from_extn( $upload[ "extn" ] ), 'guid' => $upload[ "url" ], );
		 ...
		 * @since rt-Helpdesk 0.1
		 */
		function add_contacts_to_postadd_contacts_to_post( $allemail, $post_id ) {
			...
			foreach ( $allemail as $email ) {
				...
				if ( ! empty( $postterms ) ) {
					...
					$data        = array( $attr_name => implode( ',', $postterms ), 'date_update' => current_time( 'mysql' ), 'date_update_gmt' => gmdate( 'Y-m-d H:i:s' ), 'user_updated_by' => get_current_user_id(), );
		 ...
		 * @since rt-Helpdesk 0.1
		 */
		public function process_email_to_ticket( $title, $body, $fromemail, $mailtime, $allemail, $uploaded, $mailBodyText, $check_duplicate = true, $userid = false, $messageid = "", $inreplyto = "", $references = "", $systemEmail = false, $subscriber = array() ) {
			global $rt_hd_contacts;
		...
		public function insert_post_comment( $comment_post_ID, $userid, $comment_content, $comment_author, $comment_author_email, $commenttime, $uploaded, $allemails, $dndEmails, $messageid = "", $inreplyto = "", $references = "", $subscriber = array() ) {
			...
			$data                = array( 'comment_post_ID' => $comment_post_ID, 'comment_author' => $comment_author, 'comment_author_email' => $comment_author_email, 'comment_author_url' => 'http://', 'comment_content' => $comment_content, 'comment_type' => 'mail', 'comment_parent' => 0, 'user_id' => $userid, 'comment_author_IP' => '127.0.0.1', 'comment_agent' => ' ', 'comment_date' => $commentDate, 'comment_date_gmt' => $commentDateGmt, 'comment_approved' => 1, );
			...
			$data  = array( 'date_update' => current_time( 'mysql' ), 'date_update_gmt' => gmdate( 'Y-m-d H:i:s' ), 'user_updated_by' => $userid, 'last_comment_id' => $comment_id, );
			$where = array( 'post_id' => $comment_post_ID, );
			$ticketModel->update_ticket( $data, $where );
		...
		function activecollab_task_comment_import_ajax() {
			...
			foreach ( $comment_datas as $comment_data ) {
				...
				$commentstr .= '</b></label><a class="folowup-hover" href="#editFollowup" title="Edit" data-comment-id="' . $lastCommentId . '">Edit</a>' . '<a class="folowup-hover delete" href="#deleteFollowup" title="Delete" data-comment-id="' . $lastCommentId . '">Delete</a>' . '</div><div class="large-10 columns comment-content">' . $comment_content . '</div></div></td><td class="large-2 centered">' . '<label class="comment-type"><b>' . ucfirst( $comment_type ) . '</b></label>' . '<div class="row collapse"><p class="comment-date">';
		...
		public function insert_import_mail_thread_request( $email, $threaid, $post_id, $userid = 0 ) {
			...
			$args = array( 'email' => $email, 'threadid' => $threaid, 'post_id' => $post_id, 'user_id' => $userid, 'status' => 'r' );
		...
		function rt_hd_tickets_callback( $atts ) {
			global $rt_hd_module;
			$labels = $rt_hd_module->labels;
			$a      = shortcode_atts( array( 'email' => '', 'user' => '', ), $atts );


			$args = array( 'post_type' => Rt_HD_Module::$post_type, 'post_status' => 'any', 'nopaging' => true,
