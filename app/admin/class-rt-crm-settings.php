...
/**
 * Description of Rt_CRM_Settings
 *
 * @author udit
 */
if (!class_exists('Rt_CRM_Settings')) {

	class Rt_CRM_Settings {

		public $sync_period = "00:20:00"; //"h:m:s"
		public $settingPageUrl;

		public function __construct() {
			$this->settingPageUrl = admin_url("admin.php?page=rtcrm-settings&type=crmsystememail");
		}
		...
		function ui() {
			...
			rtcrm_get_template( 'admin/settings.php', $args );
		}

		function invalid_email_notice() {
			$this->create_notice("<p> Invalid Email Address, Please update it from <a href='" . $this->settingPageUrl . "'>Setting</a> page </p>");
		}

		function set_email_notice() {
			$this->create_notice("<p>CRM System Email Address is not set, Please set email from <a href='" . $this->settingPageUrl . "'>Setting</a> page </p>");
		}
		...
		public function update_sync_meta_time($email, $replytime) {
			...
			$rows_affected = $rt_crm_mail_accounts_model->update_mail_account( array( 'last_sync_time' => $replytime ), array( 'email' => $email ) );
		...
		public function update_sync_status($email, $isSyncing) {
			...
			if ($isSyncing)
				$status = "syncing";
			global $rt_crm_mail_accounts_model;
			$rows_affected = $rt_crm_mail_accounts_model->update_mail_account( array( 'sync_status' => $status ), array( 'email' => $email ) );
			return ( !empty( $rows_affected ) );
		}

		function get_email_acc( $email ) {
			global $rt_crm_mail_accounts_model;
			$emails = $rt_crm_mail_accounts_model->get_mail_account( array( 'email' => $email ) );
			$email = false;
			if ( ! empty( $emails ) ) {
				$email = $emails[0];
			}
		...
		public function get_user_google_ac($user_id = 0) {
			...
			return $rt_crm_mail_accounts_model->get_mail_account( array( 'user_id' => $user_id ) );
		}

		public function get_accesstoken_from_email( $email, &$signature, &$email_type, &$imap_server ) {
			global $rt_crm_mail_accounts_model;
			$ac = $rt_crm_mail_accounts_model->get_mail_account( array( 'email' => $email ) );
			if(isset($ac[0])) {
				$ac = $ac[0];
			} else {
				$signature = '';
			...
			if (!$redirect_url) {
				$redirect_url = admin_url("admin.php?page=rtcrm-user-settings&type=personal");
		...
		public function update_gmail_ac_count() {
			global $rt_crm_mail_accounts_model;
			$accounts = $rt_crm_mail_accounts_model->get_all_mail_accounts();
			Rt_CRM_Utils::setAccounts( sizeof( $accounts ) );
		}
		...
		public function add_user_google_ac( $outh_token, $email, $email_data, $user_id = -1, $type = "goauth", $imap_server = NULL ) {
			...
			$rows_affected = $rt_crm_mail_accounts_model->add_mail_account( $args );
		...
		public function update_user_google_ac($outh_token, $email, $email_data) {
			...
			$rows_affected = $rt_crm_mail_accounts_model->update_mail_account( $data, $where );
			return ( !empty( $rows_affected ) );
		}
		...
		public function delete_user_google_ac($email, $user_id = -1) {
			...
			global $rt_crm_mail_accounts_model;
			$result = $rt_crm_mail_accounts_model->remove_mail_account( array( 'email' => $email, 'user_id' => $user_id ) );
			$this->update_gmail_ac_count();
			return $result;
		}

		public function update_mail_acl( $email, $token = NULL, $email_data = NULL,$user_ids = array(), $signature = '', $imap_server = NULL ) {
			global $rt_crm_mail_acl_model, $rt_crm_mail_accounts_model;
			$rt_crm_mail_acl_model->remove_acl( array( 'email' => $email ) );
			foreach ( $user_ids as $uid ) {
				$args = array(
					'allow_user' => $uid,
					'email' => $email,
				);
				$rt_crm_mail_acl_model->add_acl( $args );
			}
			...
			$rows_affected = $rt_crm_mail_accounts_model->update_mail_account( $args, array( 'email' => $email ) );
		}

		public function get_email_acl($email) {
			global $rt_crm_mail_acl_model;
			return $rt_crm_mail_acl_model->get_acl( array( 'email' => $email ) );
		}

		public function get_allow_email_address() {
			$user_id = get_current_user_id();
			global $wpdb, $rt_crm_mail_accounts_model, $rt_crm_mail_acl_model;
			$sql = $wpdb->prepare("(select * from {$rt_crm_mail_accounts_model->table_name} where user_id=%d)
                                union (select a.* from {$rt_crm_mail_acl_model->table_name} b inner join
                                {$rt_crm_mail_accounts_model->table_name} a on a.email=b.email where b.allow_user=%d)", $user_id, $user_id);
			return $wpdb->get_results($sql);
		}

		public function get_all_email_address() {
			global $rt_crm_mail_accounts_model;
			return $rt_crm_mail_accounts_model->get_all_mail_accounts();
		}

		function get_email_for_sync_debug( $email ) {
			global $wpdb, $rt_crm_mail_accounts_model;
			$sql = $wpdb->prepare( "select * from $rt_crm_mail_accounts_model->table_name where email = %s", $email );
		...
		public function get_email_for_sync() {
			sleep(5);
			global $wpdb, $rt_crm_mail_accounts_model;
			$sql = $wpdb->prepare( "select * from $rt_crm_mail_accounts_model->table_name where sync_status in ( 'syncing' ) and ( last_sync_time is NULL or addtime( last_sync_time, %s ) < NOW() ) order by last_sync_time limit 1", $this->sync_period );
			$row = $wpdb->get_row( $sql );
			if ( ! $row ) {
				$sql = "select * from $rt_crm_mail_accounts_model->table_name where not sync_status in ( 'syncing' ) order by last_sync_time limit 1";
				$row = $wpdb->get_row( $sql );
			} else {
		...
		public function insert_new_send_email($fromemail, $subject, $body, $toemail = array(), $ccemail = array(), $bccemail = array(), $attachement = array(), $refrence_id = 0, $refrence_type = "notification") {
			...
			$user_id = get_current_user_id();
			global $rt_crm_mail_outbound_model;
			$args = array(
				'user_id' => $user_id,
				'fromemail' => $fromemail,
			...
			return $rt_crm_mail_outbound_model->add_outbound_mail( $args );
		}

		public function get_new_sent_mail() {
			global $rt_crm_mail_outbound_model;
			return $rt_crm_mail_outbound_model->get_outbound_mail( array( 'sent' => 'no' ) );
		}

		public function update_sent_email($sentEmailID, $status = 'yes', $oldStatus = 'no') {
			global $rt_crm_mail_outbound_model;
			$rows_affected = $rt_crm_mail_outbound_model->update_outbound_mail( array( 'sent' => $status ), array( 'id' => $sentEmailID, 'sent' => $oldStatus ) );
			echo $rows_affected;
