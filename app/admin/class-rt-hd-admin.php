	...
	class Rt_HD_Admin {
		...
		public function __construct() {
			if ( is_admin() ) {
				...
				$this->hd_settings_tabs = array( 'my-settings' => array( 'menu_title' => __( 'My Settings' ), 'menu_slug' => 'my-settings', 'capability' => $this->author_cap ), 'admin-settings' => array( 'menu_title' => __( 'Admin Settings' ), 'menu_slug' => 'admin-settings', 'capability' => $this->admin_cap ), 'importers' => array( 'menu_title' => __( 'Importers' ), 'menu_slug' => 'importers', 'capability' => $this->editor_cap ), 'import-mapper' => array( 'menu_title' => __( 'Import Mapper' ), 'menu_slug' => 'import-mapper', 'capability' => $this->editor_cap ), 'import-logs' => array( 'menu_title' => __( 'Import Logs' ), 'menu_slug' => 'import-logs', 'capability' => $this->editor_cap ), );
		...
		function load_styles_scripts() {
			...
			if ( ( isset( $post->post_type ) && $post->post_type == Rt_HD_Module::$post_type ) || ( isset( $_REQUEST[ 'page' ] ) && in_array( $_REQUEST[ 'page' ], $pagearray ) ) || ( isset( $_REQUEST[ 'post_type' ] ) && $_REQUEST[ 'post_type' ] == Rt_HD_Module::$post_type )
			) {
			...
			if ( isset( $post->post_type ) && $post->post_type == Rt_HD_Module::$post_type && in_array( $pagenow, array( 'post.php', 'post-new.php' ) ) || ( isset( $_REQUEST[ 'page' ] ) && in_array( $_REQUEST[ 'page' ], $pagearray ) )
			) {
		...
		function settings_ui() {
			...
			<div class="wrap">
				...
				<h2><?php _e( 'Helpdesk Settings' ); ?></h2>
