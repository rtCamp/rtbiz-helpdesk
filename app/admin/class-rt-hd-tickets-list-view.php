...
if ( ! class_exists( 'Rt_HD_Tickets_List_View' ) ) {
	class Rt_HD_Tickets_List_View extends WP_List_Table {
		...
		function get_views() {
			...
			$views['all'] = "<a href='{$url}' {$class} >" . __( 'All <span class="count">(' .esc_attr(  $count ) . ')</span>' ) . '</a>';
		...
		public function get_columns() {
			...
			foreach ( $this->relations as $relation ) {
				...
				$columns[ 'rthd_' . $attr_name ] = __( esc_attr( $attr->attribute_label ) );
			}
					...
					foreach ( $columns as $column_name => $column_display_name ) {
						...
						switch ( $column_name ) {
							case 'cb':
								...
								echo '<input type="checkbox" name="' . esc_attr( $this->post_type  ). '[]" id="cb-select-' . esc_attr( $rec->rthd_post_id ). '" value="' . esc_attr( $rec->rthd_post_id ). '" />';
								echo '</th>';
								break;
							case 'rthd_post_id':
								echo '<td ' . esc_attr( $attributes ) . '>' . '<a href="' . esc_url( admin_url( 'edit.php?post_type=' . esc_attr( $this->post_type  ). '&page=rthd-add-' . esc_attr( $this->post_type ). '&' . esc_attr( $this->post_type ). '_id=' . esc_attr( $rec->rthd_post_id ) ) ) . '">#' . esc_attr( $rec->rthd_post_id ). '</a>';
								//.'< /td>';
								break;
							case 'rthd_title':
								echo '<td ' . esc_attr( $attributes ) . '>' . '<a href="' .esc_url( admin_url( 'edit.php?post_type=' . $this->post_type . '&page=rthd-add-' . $this->post_type . '&' . $this->post_type . '_id=' . $rec->rthd_post_id ) ) . '">' .esc_attr( $rec->rthd_title ) . '</a>';
								$actions = array(
							...
							case 'rthd_created_by':
								if ( ! empty( $rec->rthd_created_by ) ) {
									...
									echo '<td ' . esc_attr( $attributes ) . '><a href="' . esc_url( $url ) . '">' . esc_attr( $user->display_name  ). '</a>';
								} else {
									echo '<td ' . esc_attr( $attributes ) . '>-';
								}
								//.'< /td>';
							...
							case 'rthd_closed_by':
								if ( ! empty( $rec->rthd_closed_by ) ) {
									...
									echo '<td ' .  esc_attr( $attributes ) . '><a href="' . $url . '">' . $user->display_name . '</a>';
								} else {
									echo '<td ' .  esc_attr( $attributes ) . '>-';
								}
								//.'< /td>';
									...
									if ( $column_name == 'rthd_' . $attr_name ) {
										if ( ! empty( $rec->{'rthd_' . $attr_name} ) ) {
											if ( $attr->attribute_store_as == 'meta' ) {
												echo '<td ' . esc_attr( $attributes ) . '>' . $rec->{'rthd_' .  $attr_name };
												...
												echo esc_attr( implode( ' , ', $term_display ) );
											}
										} else {
											echo '<td ' . esc_attr( $attributes ) . '>-';
										}
