		...
		function init_importer( $field_array ) {
			...
			foreach ( $attributes as $attr ) {
				...
				$ticket_field[ $slug ] = array(
					...
					'slug'         => $slug,
					'required'     => ( isset( $attr_settings['is_required'] ) && 'yes' === $attr_settings['is_required'] ) ? true : false,
				);
...
//						$field_array[ Rtbiz_HD_Module::$post_type ] = array_merge( $ticket_field, $temp_arr );
			...
			<tr>
				...
				<td>
					...
					if ( ! empty( $results ) ) {
						...
						_e( 'No authors found', 'rtbiz-helpdesk' );
					}
			...
			if ( 'gravity' === $type ) {
				...
				foreach ( $map_data as $key => &$field ) {
					if ( isset( $field[0] ) ) {
						...
						if ( ! ( false === strpos( $field['fieldName'], 'field-' ) ) ) {
		...
		function create_tickets_from_map_data( $map_data, $gravity_lead_id, $type ) {
			...
			if ( ! $ticket_id ) {
				...
				if ( isset( $ticketstatus ) && 'new' !== $ticketstatus ) {
					$my_post                = array();
					$my_post['ID']          = $ticket_id;
