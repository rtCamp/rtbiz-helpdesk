...
if ( ! class_exists( 'Rt_HD_setup_wizard' ) ) {
	...
	class Rt_HD_setup_wizard {
		...
		public function __construct() {
			...
			add_action( 'admin_notices', 'rt_biz_hd_admin_notice_dependency_installed' );
		}
		...
		function register_setup_wizard() {
			if ( ! rt_biz_hd_check_wizard_completed() ) {
				...
				$this->screen_id = add_submenu_page( 'edit.php?post_type=' . esc_html( Rt_HD_Module::$post_type ), __( 'Setup Wizard', RT_BIZ_HD_TEXT_DOMAIN ), __( 'Setup Wizard', RT_BIZ_HD_TEXT_DOMAIN ), $admin_cap, 'rthd-setup-wizard', array(
					$this,
					'setup_wizard_ui',
						) );
			}
		...
		function mail_box_ui() {
			...
			<div class="rthd-setup-wizard-controls">
				...
				<?php rt_biz_hd_mailbox_setup_view(); ?>
			</div>
		...
		function rthd_outound_setup_wizard_callback() {
			...
			if ( ! empty( $obj_data['rthd_outgoing_email_from_name'] ) ) {
				rt_biz_hd_set_redux_settings( 'rthd_outgoing_email_from_name', $obj_data['rthd_outgoing_email_from_name'] );
			}

			if ( ! empty( $obj_data['rthd_outgoing_email_mailbox'] ) ) {
				rt_biz_hd_set_redux_settings( 'rthd_outgoing_email_mailbox', $obj_data['rthd_outgoing_email_mailbox'] );
			}
			$result['status'] = true;
			echo json_encode( $result );
			die( 0 );
								...
								<li>
									...
									<select class="rthd-setup-assignee" data="<?php echo $tm->term_id ?>"
											id="rthd_offering<?php echo $tm->term_id ?>">
												...
												//							$selected_userid = rt_biz_hd_get_offering_meta( 'default_assignee', $tm->term_id );
												//                                    if (empty($current)) {
		...
		function setup_team( $isheader = true ) {
			...
			<div>
				<div class="rthd-setup-wizard-controls rthd-setup-team-wizard-controls">
					...
					<?php } ?>
					<p class="rthd-notice"><?php _e( "There are 3 ways you can add users to your ‘Support' team. If you forget somebody now, you can add them later. You (admin) are already part of this team.", RT_BIZ_HD_TEXT_DOMAIN ) ?></p>
					...
					<div class="rthd-setup-team-settings">
						...
						<div class="rthd_wizard_container rthd-setup-wizard-row">
							...
							$count_domain_users = rt_biz_hd_search_non_helpdesk_users( $domain_name, true, true );
							?>
							...
							<div class="rthd-domain-action">
								<span id='rthd-domain-import-message' style=""> Found <?php echo sprintf( _n( '%s user', '%s users', $count_domain_users, RT_BIZ_HD_TEXT_DOMAIN ), $count_domain_users ); ?></span>
						...
						<div class="rthd_wizard_container rthd-setup-wizard-row">
							<form>
								...
								?>
								<label> 3. Add all WordPress <?php echo sprintf( _n( '(%s) user', '(%s) users', $remain_wp_users, RT_BIZ_HD_TEXT_DOMAIN ), $remain_wp_users ); ?></label>
					...
					<div class="rthd_wizard_container rthd_selected_user clearfix">
						<table class="rthd-setup-ul-text-decoration rthd-setup-list-users">
							<tr>
								<th>User</th>
								<th><span><?php _e( 'Admin', RT_BIZ_HD_TEXT_DOMAIN ); ?></span>
									<span class="rthd-tooltip">
										<i class="dashicons dashicons-info rtmicon"></i>
										<span class="rthd-tip-bottom">
											<?php _e( 'Can manage all tickets and Helpdesk settings.', RT_BIZ_HD_TEXT_DOMAIN ); ?>
										</span>
									</span>
								</th>
								<th><span><?php _e( 'Editor', RT_BIZ_HD_TEXT_DOMAIN ); ?></span>
									<span class="rthd-tooltip">
										<i class="dashicons dashicons-info rtmicon"></i>
										<span class="rthd-tip-bottom">
											<?php _e( 'Can manage all the tickets. No access to settings. ', RT_BIZ_HD_TEXT_DOMAIN ); ?>
										</span>
									</span>
								</th>
								<th class="rthd-author-row"><span><?php _e( 'Author', RT_BIZ_HD_TEXT_DOMAIN ); ?></span>
									<span class="rthd-tooltip">
										<i class="dashicons dashicons-info rtmicon"></i>
										<span class="rthd-tip-bottom">
											<?php _e( 'Can manage only the tickets assigned to them. No access to settings.', RT_BIZ_HD_TEXT_DOMAIN ); ?>
										</span>
									</span>
								</th>
								<th></th>
		...
		function get_user_from_name() {
			...
			$query = $_POST['query'];
			$results = rt_biz_hd_search_non_helpdesk_users( $query, false, false );
			$arrReturn = array();
			if ( ! empty( $results ) ) {
		...
		function rthd_creater_rtbiz_and_give_access_helpdesk() {
			...
			if ( ! empty( $_POST['email'] ) || ! empty( $_POST['ID'] ) ) {
				...
				if ( ! empty( $_POST['ID'] ) ) {
					...
					//                  $team_id = rt_biz_hd_get_default_support_team();

					if ( rt_biz_hd_give_user_access( $wpuser, Rt_Access_Control::$permissions['author']['value'], 0 ) ) {
		...
		function domain_search_and_import() {
			...
			if ( ! empty( $_POST['count'] ) && ! empty( $_POST['domain_query'] ) && ! empty( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], get_current_user_id() . 'import-user-domain' ) ) {
				if ( 'true' === $_POST['count'] ) { // return counts of users
					$arrReturn['count'] = rt_biz_hd_search_non_helpdesk_users( $_POST['domain_query'], true, true );
					$arrReturn['status'] = true;
				} else {
					$users_to_import = rt_biz_hd_search_non_helpdesk_users( $_POST['domain_query'], false, false );
					//                  $team_id = rt_biz_hd_get_default_support_team();
					$arrReturn['imported_all'] = true;
					foreach ( $users_to_import as $user ) {
						if ( rt_biz_hd_give_user_access( $user->ID, Rt_Access_Control::$permissions['author']['value'], 0 ) ) {
		...
		function offering_sync() {
			...
			rt_biz_hd_set_redux_setting( 'offering_plugin', $offering );
		...
		function assignee_save() {
			...
			if ( ! empty( $_POST['assignee'] ) ) {
				foreach ( $_POST['assignee'] as $assingee ) {
					rt_biz_hd_update_offering_meta( 'default_assignee', $assingee['user_ID'], $assingee['term_ID'] );
				}
				$arrReturn['status'] = true;
			}
			//default_assignee
			if ( ! empty( $_POST['default_assignee'] ) && is_numeric( $_POST['default_assignee'] ) ) {
				rt_biz_hd_set_redux_settings( 'rthd_default_user', $_POST['default_assignee'] );
				$arrReturn['status'] = true;
			}
		...
		function rthd_remove_user() {
			...
			if ( ! empty( $_POST['userid'] ) ) {
				...
				$rt_biz_acl_model->remove_acl( array( 'module' => RT_BIZ_HD_TEXT_DOMAIN, 'userid' => $_POST['userid'] ) );
				...
				if ( ! empty( $contact[0] ) && empty( $team_term_id ) ) {
					$user_permissions = get_post_meta( $contact[0]->ID, 'rt_biz_profile_permissions', true );
					if ( ! empty( $user_permissions[ RT_BIZ_HD_TEXT_DOMAIN ] ) ) {
						$user_permissions[ RT_BIZ_HD_TEXT_DOMAIN ] = 0;
		...
		function rthd_change_acl() {
			...
			if ( ! empty( $_POST['permission'] ) && ! empty( $_POST['userid'] ) ) {
				...
				$rt_biz_acl_model->update_acl( array( 'permission' => $_POST['permission'] ), array( 'userid' => $_POST['userid'], 'module' => RT_BIZ_HD_TEXT_DOMAIN ) );
				...
				if ( ! empty( $contact[0] ) ) {
					$user_permissions = get_post_meta( $contact[0]->ID, 'rt_biz_profile_permissions', true );
					$value = array(
						RT_BIZ_HD_TEXT_DOMAIN => $_POST['permission'],
