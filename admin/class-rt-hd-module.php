	...
	class Rt_HD_Module {
		...
		function ticket_updated_messages( $messages ) {
			...
			$messages[ self::$post_type ] = array(
				...
				1  => __( 'Ticket updated.', RT_BIZ_HD_TEXT_DOMAIN ),
				2  => __( 'Custom field updated.', RT_BIZ_HD_TEXT_DOMAIN ),
				3  => __( 'Custom field deleted.', RT_BIZ_HD_TEXT_DOMAIN ),
				4  => __( 'Ticket updated.', RT_BIZ_HD_TEXT_DOMAIN ),
				/* translators: %s: date and time of the revision */
				5  => isset( $_GET['revision'] ) ? sprintf( __( 'Ticket restored to revision from %s', RT_BIZ_HD_TEXT_DOMAIN ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
				6  => __( 'Ticket published.', RT_BIZ_HD_TEXT_DOMAIN ),
				7  => __( 'Ticket saved.', RT_BIZ_HD_TEXT_DOMAIN ),
				8  => __( 'Ticket submitted.', RT_BIZ_HD_TEXT_DOMAIN ),
				10 => __( 'Ticket draft updated.', RT_BIZ_HD_TEXT_DOMAIN ),
			);

			/*if ( $post_type_object->publicly_queryable ) {
				$permalink = get_permalink( $post->ID );

				$view_link = sprintf( ' <a href="%s">%s</a>', esc_url( $permalink ), __( 'View Ticket', RT_BIZ_HD_TEXT_DOMAIN ) );
				$messages[ $post_type ][1] .= $view_link;
				$messages[ $post_type ][6] .= $view_link;
				$messages[ $post_type ][9] .= $view_link;

				$preview_permalink = esc_url( add_query_arg( 'preview', 'true', $permalink ) );
				$preview_link = sprintf( ' <a target="_blank" href="%s">%s</a>', esc_url( $preview_permalink ), __( 'Preview Ticket', RT_BIZ_HD_TEXT_DOMAIN ) );
		...
		function get_custom_labels() {
			...
			$this->labels = array(
				'name'          => __( 'Ticket', RT_BIZ_HD_TEXT_DOMAIN ),
				'singular_name' => __( 'Ticket', RT_BIZ_HD_TEXT_DOMAIN ),
				'menu_name'     => 'Helpdesk',
				'all_items'     => __( 'Tickets', RT_BIZ_HD_TEXT_DOMAIN ),
				'add_new'       => __( 'Add New Ticket', RT_BIZ_HD_TEXT_DOMAIN ),
				'add_new_item'  => __( 'Add New Ticket', RT_BIZ_HD_TEXT_DOMAIN ),
				'new_item'      => __( 'Add New Ticket', RT_BIZ_HD_TEXT_DOMAIN ),
				'edit_item'     => __( 'Edit Ticket', RT_BIZ_HD_TEXT_DOMAIN ),
				'view_item'     => __( 'View Ticket', RT_BIZ_HD_TEXT_DOMAIN ),
				'search_items'  => __( 'Search Tickets', RT_BIZ_HD_TEXT_DOMAIN ),
			);
		...
		function get_custom_statuses() {
			$this->statuses = array(
				array(
					'slug'        => 'hd-unanswered',
					'name'        => __( 'Unanswered', RT_BIZ_HD_TEXT_DOMAIN ),
					'description' => __( 'Ticket is unanswered. It needs to be replied. The default state.', RT_BIZ_HD_TEXT_DOMAIN ),
				),
				array(
					'slug'        => 'hd-answered',
					'name'        => __( 'Answered', RT_BIZ_HD_TEXT_DOMAIN ),
					'description' => __( 'Ticket is answered. Expecting further communication from client', RT_BIZ_HD_TEXT_DOMAIN ),
				),
				array(
					'slug'        => 'hd-archived',
					'name'        => __( 'Archived', RT_BIZ_HD_TEXT_DOMAIN ),
					'description' => __( 'Ticket is archived. Client can re-open if they wish to.', RT_BIZ_HD_TEXT_DOMAIN ),
				),
			);
		...
		function get_custom_menu_order() {
			...
			$this->custom_menu_order = array(
				...
				esc_url( 'edit.php?post_type=' . rt_biz_get_contact_post_type() . '&rt_contact_group=customer&module=' . RT_BIZ_HD_TEXT_DOMAIN ),
				esc_url( 'edit.php?post_type=' . rt_biz_get_contact_post_type() . '&rt_contact_group=staff&module=' . RT_BIZ_HD_TEXT_DOMAIN ),
		...
		function p2p_hook_for_rthd_post_filter( $args, $ctype, $post  ) {
			...
			if ( $ctype->name == self::$post_type.'_to_'.rt_biz_get_contact_post_type() ) {
				$exclude = array();
				// ACL
				$result  = $wpdb->get_col( 'SELECT p2p_from FROM '.$wpdb->prefix."p2p WHERE p2p_type = '".rt_biz_get_contact_post_type()."_to_user' AND p2p_to in (SELECT DISTINCT(userid) FROM ".$rt_biz_acl_model->table_name." where module = '".RT_BIZ_HD_TEXT_DOMAIN."' and permission != 0 )" );
		...
		function register_custom_post( $menu_position ) {
			...
			$logo = apply_filters( 'rthd_helpdesk_logo', RT_HD_URL . 'public/img/hd-16X16.png' );
		...
		function hooks() {
			...
			if ( rt_biz_hd_get_redux_adult_filter() && isset( $_GET['post_type'] ) && $_GET['post_type'] == self::$post_type ) {
		...
		function adult_post_filter( $query ) {
			...
			if ( is_admin() && $query->query['post_type'] == self::$post_type && strpos( $_SERVER['REQUEST_URI'], '/wp-admin/edit.php' ) !== false ) {
				...
				$pref = rt_biz_hd_get_user_adult_preference( $current_user );
				if ( 'yes' == $pref ) {
					$meta_q = array(
						'relation' => 'OR',
						array(
		...
		function custom_pages_order( $menu_order ) {
			...
			if ( isset( $submenu[ 'edit.php?post_type=' . self::$post_type ] ) && ! empty( $submenu[ 'edit.php?post_type=' . self::$post_type ] ) ) {
				...
				$new_index = 5;
				$wizard_completed = rt_biz_hd_check_wizard_completed();
				...
				/*foreach ( $module_menu as $p_key => $menu_item ) {
	                if ( rt_biz_hd_check_wizard_completed() || ( ! rt_biz_hd_check_wizard_completed() && in_array( 'rthd-setup-wizard', $menu_item ) ) ) {
		                $submenu[ 'edit.php?post_type=' . self::$post_type ][ $new_index ] = $menu_item;
		                unset( $module_menu[ $p_key ] );
		                $new_index += 5;
	                }
