...
if ( ! class_exists( 'Rt_HD_Contacts' ) ) {
	...
	class Rt_HD_Contacts {
		...
		function hooks() {
			...
			add_action( 'rtbiz_after_delete_staff_acl_remove-' . RT_BIZ_HD_TEXT_DOMAIN, array(
				$this,
		...
		function rthd_ticket_listing_metabox( $post ) {
			if ( ! empty( $_REQUEST['module'] ) && RT_BIZ_HD_TEXT_DOMAIN == $_REQUEST['module'] ) {
		...
		function display_custom_views( $views ) {
			if ( ! empty( $_REQUEST['module'] ) && RT_BIZ_HD_TEXT_DOMAIN == $_REQUEST['module'] && isset( $_REQUEST['rt_contact_group'] ) ) {
				if ( 'staff' == $_REQUEST['rt_contact_group'] ) {
					if ( ! isset( $_GET['role'] ) ) { $class = ' class="current"'; } else { $class = ''; }
					$temp_view['All'] = "<a href='edit.php?post_type=" . rt_biz_get_contact_post_type() . '&rt_contact_group=staff&module=' . RT_BIZ_HD_TEXT_DOMAIN . "' $class>" . __( 'All' ) . '</a>';
					if ( isset( $_GET['role'] ) && 'admin' == $_GET['role'] ) { $class = ' class="current"'; } else { $class = ''; }
					$temp_view['Admin'] = "<a href='edit.php?post_type=" . rt_biz_get_contact_post_type() . '&rt_contact_group=staff&module=' . RT_BIZ_HD_TEXT_DOMAIN . "&role=admin' $class>" . __( 'Admin' ) . '</a>';
					if ( isset( $_GET['role'] ) && 'editor' == $_GET['role'] ) { $class = ' class="current"'; } else { $class = ''; }
					$temp_view['Editor'] = "<a href='edit.php?post_type=" . rt_biz_get_contact_post_type() . '&rt_contact_group=staff&module=' . RT_BIZ_HD_TEXT_DOMAIN . "&role=editor' $class>" . __( 'Editor' ) . '</a>';
					if ( isset( $_GET['role'] ) && 'author' == $_GET['role'] ) { $class = ' class="current"'; } else { $class = ''; }
					$temp_view['Author'] = "<a href='edit.php?post_type=" . rt_biz_get_contact_post_type() . '&rt_contact_group=staff&module=' . RT_BIZ_HD_TEXT_DOMAIN . "&role=author' $class>" . __( 'Author' ) . '</a>';
				} elseif ( 'customer' == $_REQUEST['rt_contact_group'] ) {
					if ( ! isset( $_GET['tickets'] ) ) { $class = ' class="current"'; } else { $class = ''; }
					$temp_view['All'] = "<a href='edit.php?post_type=" . rt_biz_get_contact_post_type() . '&rt_contact_group=customer&module=' . RT_BIZ_HD_TEXT_DOMAIN . "' $class>" . __( 'All' ) . '</a>';
					//$temp_view['All'] = "<a href='edit.php?post_type=" . rt_biz_get_contact_post_type() . "&rt_contact_group=customer&module=" . RT_BIZ_HD_TEXT_DOMAIN . "&tickets=all' $class>" . sprintf( _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', 0, RT_BIZ_HD_TEXT_DOMAIN ), number_format_i18n( 0 ) ) . '</a>';
					if ( isset( $_GET['tickets'] ) && 'yes' == $_GET['tickets'] ) { $class = ' class="current"'; } else { $class = ''; }
					$temp_view['With_Ticket'] = "<a href='edit.php?post_type=" . rt_biz_get_contact_post_type() . '&rt_contact_group=customer&module=' . RT_BIZ_HD_TEXT_DOMAIN . "&tickets=yes' $class>" . __( 'With Tickets' ) . '</a>';
					//$temp_view['With_Ticket'] = "<a href='edit.php?post_type=" . rt_biz_get_contact_post_type() . "&rt_contact_group=customer&module=" . RT_BIZ_HD_TEXT_DOMAIN . "&tickets=yes' $class>" . sprintf( _nx( 'With Tickets <span class="count">(%s)</span>', 'With Tickets <span class="count">(%s)</span>', 0, RT_BIZ_HD_TEXT_DOMAIN ), number_format_i18n( 0 ) ) . '</a>';
					if ( isset( $_GET['tickets'] ) && 'no' == $_GET['tickets'] ) { $class = ' class="current"'; } else { $class = ''; }
					$temp_view['Without_Ticket'] = "<a href='edit.php?post_type=" . rt_biz_get_contact_post_type() . '&rt_contact_group=customer&module=' . RT_BIZ_HD_TEXT_DOMAIN . "&tickets=no' $class>" . __( 'Without Tickets' ) . '</a>';
					//$temp_view['Without_Ticket'] = "<a href='edit.php?post_type=" . rt_biz_get_contact_post_type() . "&rt_contact_group=customer&module=" . RT_BIZ_HD_TEXT_DOMAIN . "&tickets=no' $class>" . sprintf( _nx( 'Without Tickets <span class="count">(%s)</span>', 'Without Tickets <span class="count">(%s)</span>', 0, RT_BIZ_HD_TEXT_DOMAIN ), number_format_i18n( 0 ) ) . '</a>';
				}
				$views = $temp_view;
			}
			return $views;
		...
		function rthd_redirect_post_location_filter( $location ) {
			if ( ! empty( $_POST['post_type'] ) && rt_biz_get_contact_post_type() == $_POST['post_type']
			     &&  strpos( $_POST['_wp_http_referer'], 'module=' . RT_BIZ_HD_TEXT_DOMAIN ) !== false ) {
				$location = add_query_arg( 'module', RT_BIZ_HD_TEXT_DOMAIN, $location );
			}
			return $location;
		}
		...
		function save_helpdesk_role( $post_id, $post ) {
			...
			if ( isset( $_REQUEST['rtbiz_action'] ) && 'rtbiz_helpdesk_role_updated' == $_REQUEST['rtbiz_action'] ) {
				...
				$_REQUEST['rt_biz_profile_permissions'][ RT_BIZ_TEXT_DOMAIN ] = $_REQUEST['rt_biz_profile_permissions'][ RT_BIZ_HD_TEXT_DOMAIN ];
				...
				foreach ( $contactIds as $contactId ) {
					...
					$user_permissions[ RT_BIZ_HD_TEXT_DOMAIN ]  = $profile_permissions[ RT_BIZ_HD_TEXT_DOMAIN ];
		...
		function rthd_add_setting_to_rtbiz_user( $fields ) {
			...
			if ( ( ( ! empty( $_REQUEST['module'] ) &&  RT_BIZ_HD_TEXT_DOMAIN == $_REQUEST['module'] )
			       || ( isset( $_POST['_wp_http_referer'] ) && strpos( $_POST['_wp_http_referer'], 'module=' . RT_BIZ_HD_TEXT_DOMAIN ) !== false ) )
		...
		function get_user_from_email( $email ) {
			...
			if ( ! $userid && ! is_wp_error( $userid ) ) {
				...
				rt_biz_hd_wp_new_user_notification( $userid, $random_password );
			}
		...
		function contacts_diff_on_ticket( $post_id, $newTicket ) {
			...
			$diff = rt_biz_hd_text_diff( $oldContactsString, $newContactsSring );
			if ( $diff ) {
