...
if ( ! class_exists( 'Rt_Biz_Helpdesk_Admin' ) ) {
	...
	class Rt_Biz_Helpdesk_Admin {
		...
		public function __construct() {
			...
			$this->admin_cap = rt_biz_get_access_role_cap( RT_BIZ_HD_TEXT_DOMAIN, 'admin' );
			$this->editor_cap = rt_biz_get_access_role_cap( RT_BIZ_HD_TEXT_DOMAIN, 'editor' );
			$this->author_cap = rt_biz_get_access_role_cap( RT_BIZ_HD_TEXT_DOMAIN, 'author' );
		...
		function add_people_menu() {
			if ( rt_biz_hd_check_wizard_completed() ) {
				add_submenu_page( 'edit.php?post_type=' . Rt_HD_Module::$post_type, __( 'Customers' ), __( 'Customers' ), rt_biz_get_access_role_cap( RT_BIZ_HD_TEXT_DOMAIN, 'editor' ), esc_url( 'edit.php?post_type=' . rt_biz_get_contact_post_type() . '&rt_contact_group=customer&module=' . RT_BIZ_HD_TEXT_DOMAIN ) );
				add_submenu_page( 'edit.php?post_type=' . Rt_HD_Module::$post_type, __( 'Staff' ), __( 'Staff' ), rt_biz_get_access_role_cap( RT_BIZ_HD_TEXT_DOMAIN, 'editor' ), esc_url( 'edit.php?post_type=' . rt_biz_get_contact_post_type() . '&rt_contact_group=staff&module=' . RT_BIZ_HD_TEXT_DOMAIN ) );
				...
				/* add_submenu_page( 'edit.php?post_type=' . esc_html( Rt_HD_Module::$post_type ), __( 'Companies' ), __( '--- Companies' ), rt_biz_get_access_role_cap( RT_BIZ_HD_TEXT_DOMAIN, 'admin' ), 'edit.php?post_type=' . rt_biz_get_company_post_type() );
				  add_submenu_page( 'edit.php?post_type=' . esc_html( Rt_HD_Module::$post_type ), __( 'Access Control' ), __( '--- Access Control' ), rt_biz_get_access_role_cap( RT_BIZ_HD_TEXT_DOMAIN, 'admin' ), Rt_Biz::$access_control_slug, array(
				  $rt_access_control,
				  'acl_settings_ui'
				  ) ); */
		...
		function tab_content( $screen, $tab ) {
			switch ( $tab['id'] ) {
				case 'hd_acl_overview':
					...
					<p>
					<ul>
						<li><?php _e( 'Admin - can manage all tickets and has full access to settings.', RT_BIZ_HD_TEXT_DOMAIN ) ?></li>
						<li><?php _e( 'Editor - can manage all tickets but has no access to settings.', RT_BIZ_HD_TEXT_DOMAIN ) ?></li>
						<li><?php _e( 'Author - can only manage tickets assigned to himself/herself.', RT_BIZ_HD_TEXT_DOMAIN ) ?></li>
						<li><?php _e( 'No Role - has no access to ticket backend but has read-only acces the web interface of the ticket.', RT_BIZ_HD_TEXT_DOMAIN ) ?></li>
						<li><?php _e( 'Group Access - has same access as his/her team has.', RT_BIZ_HD_TEXT_DOMAIN ) ?></li>
					</ul>
					</p> <?php
					break;
			}
		...
		function load_styles_scripts( $hook ) {
			...
			if ( ( in_array( $pagenow, array( 'edit.php', 'post.php', 'post-new.php' ) ) && Rt_HD_Module::$post_type == $rthd_post_type )
				...
				if ( isset( $post->post_type ) && $post->post_type == Rt_HD_Module::$post_type ) {
					...
					wp_enqueue_script( 'jquery-ui-timepicker-addon', RT_HD_URL . 'admin/js/vendors/jquery-ui-timepicker-addon.js', array(
						'jquery-ui-datepicker',
						'jquery-ui-slider',
							), RT_BIZ_HD_VERSION, true );
				...
				wp_enqueue_media();
				wp_enqueue_style( 'rthd-admin-css', RT_HD_URL . 'admin/css/admin.css', array(), RT_BIZ_HD_VERSION );
				wp_enqueue_script( 'rthd-admin-js', RT_HD_URL . 'admin/js/helpdesk-admin-min.js', array( 'jquery' ), RT_BIZ_HD_VERSION, true );

			}

			if ( isset( $rthd_post_type ) && in_array( $rthd_post_type, array( rt_biz_get_contact_post_type() ) ) ) {
				wp_enqueue_script( 'rthd-menu-hack-js', RT_HD_URL . 'admin/js/rt-custom-status.js', array( 'jquery' ), time(), true );
				wp_localize_script( 'rthd-menu-hack-js', 'rthd_menu', Rt_HD_Module::$post_type );
				$query_arg = '';
				if ( ! empty( $_GET['rt_contact_group'] ) ) {
					$query_arg = '&rt_contact_group=' . $_GET['rt_contact_group'] . '&module=' . RT_BIZ_HD_TEXT_DOMAIN;
				} else {
					if ( isset( $_REQUEST['post'] ) ) {
						...
						$query_arg .= '&module=' . RT_BIZ_HD_TEXT_DOMAIN;
					}
				}
		...
		function custom_upload_dir( $args ) {
			$args['path'] = $args['basedir'] . '/' . RT_BIZ_HD_TEXT_DOMAIN . $args['subdir'];
			$args['url'] = $args['baseurl'] . '/' . RT_BIZ_HD_TEXT_DOMAIN . $args['subdir'];
			return $args;
		}
